...
use un_motion_interfaces::{ImageFrame, ImageInputSource, ImageResizeOptions, PixelFormat, ResizeAxis, RgbaColor};
#[cfg(not(windows))]
...
pub(crate) struct CoreImageMotionFrameSource<I> {
	...
	input_denoise_mode: InputDenoiseMode,
	input_denoise_temporal_iir_hz: f64,
	input_denoise_state: InputDenoiseState,
...
struct SubmittedImageFrame {
	...
	debug_input_image_path: Option<String>,
}

impl SubmittedImageFrame {
	fn from_image(image: &ImageFrame, debug_input_image_path: Option<String>) -> Self {
		Self {
	...
	I: ImageInputSource + Send + 'static,
{
	fn next_frame(&mut self) -> anyhow::Result<Option<UNMotionFrame>> {
		...
		let Some(mut image) = self.input.next_image_frame()? else {
			return Ok(None);
		};
		...
		if self.input_denoise_mode != InputDenoiseMode::Off {
			image = denoise_rgb8_image(
				&image,
				self.input_denoise_mode,
				self.input_denoise_temporal_iir_hz,
				&mut self.input_denoise_state,
			)?;
		}
		let debug_input_image_path = self.write_debug_input_image_if_active(&image);
		let submitted = SubmittedImageFrame::from_image(&image, debug_input_image_path);
		if self.live_stream_mode {
	...
	I: ImageInputSource + Send + 'static,
{
	...
	fn process_native_output_for_submitted(&mut self, submitted: SubmittedImageFrame, native: &NativeMediaPipeOutput) -> UNMotionFrame {
		...
		self.push_debug_analysis_sample(&submitted, native, &frame);
		frame
	}

	fn write_debug_input_image_if_active(&self, image: &ImageFrame) -> Option<String> {
		if !self.telemetry.debug.is_active() {
			return None;
		}
		let output_dir = self.telemetry.debug.output_dir()?;
		let file_name = format!(
			"{}-seq{}-ts{}.png",
			self.input_component.replace([':', '/', '\\'], "_"),
			image.metadata.sequence,
			image.metadata.capture_timestamp_ns
		);
		let path = output_dir.join(file_name);
		match write_debug_rgb_png(image, &path) {
			...
			Err(error) => {
				warn!(
					...
					error = %error,
					path = %path.display(),
					"failed to write analysis input image",
				);
				None
			}
		}
	...
	fn push_debug_analysis_sample(&self, submitted: &SubmittedImageFrame, native: &NativeMediaPipeOutput, frame: &UNMotionFrame) {
		if !self.telemetry.debug.is_active() {
			return;
		}
		let sample = serde_json::json!({
			"kind": "mediapipe-analysis-frame",
			"schema": "un-motion.dev.mediapipe-analysis.v1",
			"sourceId": &self.telemetry.source_id,
			"inputComponent": &self.input_component,
			"runtimeEngine": &self.runtime_engine,
			"postProcess": &self.post_process_component,
			"input": {
				"sequence": submitted.sequence,
				"captureTimestampNs": submitted.capture_timestamp_ns,
				"timestampMs": submitted.timestamp_ms,
				"width": submitted.width,
				"height": submitted.height,
				"sourceLabel": submitted.source_label.as_deref(),
				"imagePath": submitted.debug_input_image_path.as_deref(),
			},
			"native": native_output_debug_json(native),
			"unmotionFrame": frame,
		});
		self.telemetry.debug.push_sample(sample);
	}
...
enum InputDenoiseMode {
	#[default]
	Off,
	TemporalIir,
}

impl InputDenoiseMode {
	fn from_config(value: &str) -> Self {
		match value.trim().to_ascii_lowercase().as_str() {
			"temporal-iir" | "temporal_iir" | "temporal-iir-10hz" | "temporal_iir_10hz" | "temporal-iir-8hz" | "temporal_iir_8hz"
			| "temporal-iir-6hz" | "temporal_iir_6hz" | "temporal-iir-4hz" | "temporal_iir_4hz" | "temporal-iir-2hz"
			| "temporal_iir_2hz" => Self::TemporalIir,
			_ => Self::Off,
		}
	}
}
...
#[derive(Default)]
struct InputDenoiseState {
	previous_temporal: Option<TemporalIirFrame>,
}

struct TemporalIirFrame {
	width: u32,
	height: u32,
	stride_bytes: u32,
	capture_timestamp_ns: u64,
...
fn denoise_rgb8_image(
	image: &ImageFrame,
	mode: InputDenoiseMode,
	temporal_iir_hz: f64,
	state: &mut InputDenoiseState,
) -> anyhow::Result<ImageFrame> {
	if mode == InputDenoiseMode::Off {
		return Ok(image.clone());
	}
	if image.pixel_format != PixelFormat::Rgb8 {
		anyhow::bail!("input denoise currently requires Rgb8 ImageFrame input");
	}
	if image.width < 3 || image.height < 3 {
		return Ok(image.clone());
	}
	match mode {
		InputDenoiseMode::Off => Ok(image.clone()),
		InputDenoiseMode::TemporalIir => Ok(apply_temporal_iir_denoise(image, state, temporal_iir_hz)),
	}
}

fn apply_temporal_iir_denoise(input: &ImageFrame, state: &mut InputDenoiseState, cutoff_hz: f64) -> ImageFrame {
	const DEFAULT_DT_SECONDS: f64 = 1.0 / 60.0;
	let source = &input.data;
	let compatible_previous = state.previous_temporal.as_ref().filter(|previous| {
		previous.width == input.width
			&& previous.height == input.height
			&& previous.stride_bytes == input.stride_bytes
			&& previous.data.len() == source.len()
			&& input.metadata.capture_timestamp_ns > previous.capture_timestamp_ns
	});
	if compatible_previous.is_none() {
		let data = source.clone();
		state.previous_temporal = Some(TemporalIirFrame {
			width: input.width,
			height: input.height,
			stride_bytes: input.stride_bytes,
			capture_timestamp_ns: input.metadata.capture_timestamp_ns,
			data: data.clone(),
		});
		return image_frame_with_data(input, data);
	};
	let previous = state.previous_temporal.as_mut().expect("compatible previous temporal frame");
	let dt_seconds =
		((input.metadata.capture_timestamp_ns - previous.capture_timestamp_ns) as f64 / 1_000_000_000.0).clamp(1.0 / 240.0, 0.25);
	let dt_seconds = if dt_seconds.is_finite() { dt_seconds } else { DEFAULT_DT_SECONDS };
	let rc = 1.0 / (2.0 * std::f64::consts::PI * cutoff_hz.max(0.1));
	let alpha = ((dt_seconds / (rc + dt_seconds)).clamp(0.0, 1.0) * 256.0).round() as i32;
	let inverse_alpha = 256 - alpha;
	for (index, current) in source.iter().copied().enumerate() {
		let filtered = (previous.data[index] as i32 * inverse_alpha + current as i32 * alpha + 128) >> 8;
		previous.data[index] = filtered.clamp(0, 255) as u8;
	}
	previous.capture_timestamp_ns = input.metadata.capture_timestamp_ns;
	let data = previous.data.clone();
	image_frame_with_data(input, data)
}

fn image_frame_with_data(input: &ImageFrame, data: Vec<u8>) -> ImageFrame {
	ImageFrame {
	...
	I: ImageInputSource + Send + 'static,
{
	...
	Ok(CoreImageMotionFrameSource {
		...
		input_denoise_mode: InputDenoiseMode::from_config(&config.input_denoise_mode),
		input_denoise_temporal_iir_hz: config.input_denoise_temporal_iir_hz,
		input_denoise_state: InputDenoiseState::default(),
...
fn media_pipe_post_process_config(
	...
	MediaPipePostProcessConfig {
		...
		rules: MediaPipePostProcessRules {
			anatomical_constraints: post_process.post_process_rules.anatomical_constraints,
...
fn native_output_debug_json(native: &NativeMediaPipeOutput) -> serde_json::Value {
	serde_json::json!({
		"returnCode": native.return_code,
		"resultTimestampMs": native.result_timestamp_ms,
		"pose": {
			"landmarkCount": native.pose.landmark_count,
			"worldLandmarkCount": native.pose.world_landmark_count,
			"confidence": native.pose.confidence,
			"landmarks": landmarks_debug_json(&native.pose.landmarks, native.pose.landmark_count),
			"worldLandmarks": landmarks_debug_json(&native.pose.world_landmarks, native.pose.world_landmark_count),
		},
		"hands": {
			"handCount": native.hands.hand_count,
			"hands": native.hands.hands.iter().take(native.hands.hand_count as usize).map(|hand| serde_json::json!({
				"landmarkCount": hand.landmark_count,
				"worldLandmarkCount": hand.world_landmark_count,
				"handednessScore": hand.handedness_score,
				"handednessIsRight": hand.handedness_is_right,
				"confidence": hand.confidence,
				"landmarks": landmarks_debug_json(&hand.landmarks, hand.landmark_count),
				"worldLandmarks": landmarks_debug_json(&hand.world_landmarks, hand.world_landmark_count),
			})).collect::<Vec<_>>(),
		},
		"face": {
			"landmarkCount": native.face.landmark_count,
			"confidence": native.face.confidence,
			"matrixRows": native.face.matrix_rows,
			"matrixCols": native.face.matrix_cols,
			"matrix": native.face.matrix,
			"landmarks": landmarks_debug_json(&native.face.landmarks, native.face.landmark_count),
		},
		"holistic": {
			"poseLandmarkCount": native.holistic.pose.landmark_count,
			"leftHandLandmarkCount": native.holistic.left_hand.landmark_count,
			"rightHandLandmarkCount": native.holistic.right_hand.landmark_count,
			"faceLandmarkCount": native.holistic.face.landmark_count,
			"poseLandmarks": landmarks_debug_json(&native.holistic.pose.landmarks, native.holistic.pose.landmark_count),
			"poseWorldLandmarks": landmarks_debug_json(&native.holistic.pose.world_landmarks, native.holistic.pose.world_landmark_count),
			"leftHandLandmarks": landmarks_debug_json(&native.holistic.left_hand.landmarks, native.holistic.left_hand.landmark_count),
			"leftHandWorldLandmarks": landmarks_debug_json(&native.holistic.left_hand.world_landmarks, native.holistic.left_hand.world_landmark_count),
			"rightHandLandmarks": landmarks_debug_json(&native.holistic.right_hand.landmarks, native.holistic.right_hand.landmark_count),
			"rightHandWorldLandmarks": landmarks_debug_json(&native.holistic.right_hand.world_landmarks, native.holistic.right_hand.world_landmark_count),
			"faceLandmarks": landmarks_debug_json(&native.holistic.face.landmarks, native.holistic.face.landmark_count),
		},
	})
}

fn landmarks_debug_json(landmarks: &[un_motion_mediapipe_native::NativeLandmark], count: u32) -> Vec<[f32; 5]> {
	landmarks
		.iter()
		.take(count as usize)
		.map(|landmark| [landmark.x, landmark.y, landmark.z, landmark.visibility, landmark.presence])
		.collect()
}

fn write_debug_rgb_png(image: &ImageFrame, path: &Path) -> anyhow::Result<()> {
	if image.pixel_format != PixelFormat::Rgb8 {
		anyhow::bail!("analysis input image dump supports RGB8 only, got {:?}", image.pixel_format);
	}
	let row_bytes = image.width.saturating_mul(3) as usize;
	let stride = image.stride_bytes as usize;
	...
	let bytes = if stride == row_bytes {
		...
		for y in 0..height {
			...
			let end = start.saturating_add(row_bytes).min(image.data.len());
			if end > start {
				packed.extend_from_slice(&image.data[start..end]);
			}
		}
...
mod tests {
	...
	#[test]
	fn temporal_iir_10hz_uses_previous_filtered_frame() {
		let first = ImageFrame::new_rgb8(1, 1, "test", 3, 3, vec![0; 27]).expect("frame");
		let mut second = ImageFrame::new_rgb8(2, 33_333_333, "test", 3, 3, vec![100; 27]).expect("frame");
		second.metadata.capture_timestamp_ns = 33_333_333;
		let mut state = InputDenoiseState::default();

		let first_out = denoise_rgb8_image(&first, InputDenoiseMode::TemporalIir, 10.0, &mut state).expect("denoise");
		let second_out = denoise_rgb8_image(&second, InputDenoiseMode::TemporalIir, 10.0, &mut state).expect("denoise");

		assert_eq!(first_out.data[12], 0);
		assert!(second_out.data[12] > 0);
		assert!(second_out.data[12] < 100);
	}
}
