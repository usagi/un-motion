...
use un_motion_runtime::{
	MirrorMode, ModifierConfig, ModifierPipeline, MotionFrameStreamConfig, MotionFrameStreamWorkerHandle, MotionFrameStreamWorkerMessage,
	...
	ZenohOutputWorkerHandle, spawn_motion_frame_stream_worker, spawn_vmc_output_worker, spawn_zenoh_output_worker,
};
...
const OUTPUT_PENDING_FRAME_CAPACITY: usize = 256;
...
pub(crate) fn core_runtime_config_from_document(document: &CoreProfileDocument) -> anyhow::Result<CoreRuntimeConfig> {
	...
	let fps = runtime
		.and_then(|r| r.fps)
		.unwrap_or_else(|| default_output_fps_from_streams(&frame_streams))
		.clamp(1, MAX_FPS);
...
fn default_output_fps_from_streams(frame_streams: &[CoreMotionFrameStreamConfig]) -> u32 {
	frame_streams
		.iter()
		.map(|stream| stream.input_fps)
		.max()
...
fn core_frame_stream_config(
	...
	CoreMotionFrameStreamConfig {
		...
		input_fps: pipeline
			.and_then(|p| p.input_fps)
			.or_else(|| runtime.and_then(|r| r.fps))
			.unwrap_or(DEFAULT_FPS)
			.clamp(1, MAX_FPS),
		input_width,
		input_height,
		input_pixel_format: pipeline.and_then(|p| p.input_pixel_format.clone()),
		input_repeat: pipeline.and_then(|p| p.input_repeat).unwrap_or(false),
...
fn run_core_runtime(
	...
	// 正式経路: MotionFrameSource が生成した UNMotionFrame を両 output worker に流す。
	// `latest_unmotion_frame` は calibration / face model 用の最新スナップショット、
	// `pending_unmotion_frames` は output cadence 用の bounded queue。
	let mut latest_unmotion_frame: Option<un_motion_frame::UNMotionFrame> = None;
	let mut pending_unmotion_frames: VecDeque<un_motion_frame::UNMotionFrame> = VecDeque::new();
	...
	loop {
		...
		// `open_motion_frame_source` が失敗した場合は worker handle が空のまま Error メッセージだけが
		// frame_stream_rx に流される。これを無視すると Capturer は `health: streaming` を維持したまま
		// 「frame 0 / packet 0」状態となり、ユーザーから見て「何故か推論が動いていない」
		// 沈黙クラッシュになる。常に drain する。
		let received_unmotion = drain_frame_stream_messages(
			&workers.frame_stream_rx,
			&mut latest_states,
			&mut latest_unmotion_frame,
			&mut pending_unmotion_frames,
			&mut packet_count,
			&mut health,
			&active_profile_id,
			frame_count,
		...
		if cadence.mark_due(now) {
			if let Some(frame) = pending_unmotion_frames.pop_front() {
				frame_count = frame_count.saturating_add(1);
				if let Some(output) = &workers.vmc_output
					&& output.send(frame.clone()).is_err()
				{
					health = "output disconnected".to_string();
				}
				if let Some(output) = &workers.zenoh_output
					&& output.send(frame.clone()).is_err()
				{
...
fn initial_stream_states(config: &CoreRuntimeConfig) -> HashMap<StreamId, StreamState> {
	config
		.frame_streams
		.iter()
		.map(|stream| (stream.stream_id.clone(), StreamState::new(stream.stream_id.clone())))
		.collect()
}
...
fn collect_pending_neutral_calibration(pending: &mut Option<PendingNeutralCalibration>, latest_states: &HashMap<StreamId, StreamState>) {
...
fn push_neutral_sample(calibration: &mut PendingNeutralCalibration, latest_states: &HashMap<StreamId, StreamState>) -> bool {
...
fn collect_pending_face_pose_model(pending: &mut Option<PendingFacePoseModel>, latest_unmotion_frame: &Option<UNMotionFrame>) {
...
fn push_face_pose_model_sample(model: &mut PendingFacePoseModel, latest_unmotion_frame: &Option<UNMotionFrame>) -> bool {
...
fn summarize_neutral_rotations(samples: &BTreeMap<String, Vec<[f32; 4]>>, _pose: NeutralCalibrationPose) -> BTreeMap<String, [f32; 4]> {
	samples
		.iter()
		.filter_map(|(key, values)| {
			...
			average_quaternions(values).map(|actual| (key.clone(), actual))
		})
		.collect()
}

fn neutral_calibration_key_enabled(key: &str) -> bool {
	// Limb rotations are already posed outputs. Capturing them from a U/T/I pose
	// creates pose-dependent offsets that corrupt otherwise-good arm and hand modifier.
...
#[allow(clippy::too_many_arguments)]
fn drain_frame_stream_messages(
	frame_stream_rx: &Receiver<MotionFrameStreamWorkerMessage>,
	latest_states: &mut HashMap<StreamId, StreamState>,
	latest_unmotion_frame: &mut Option<un_motion_frame::UNMotionFrame>,
	pending_unmotion_frames: &mut VecDeque<un_motion_frame::UNMotionFrame>,
	...
	loop {
		match frame_stream_rx.try_recv() {
			Ok(MotionFrameStreamWorkerMessage::Poll(poll)) => {
				received = true;
				*packet_count = packet_count.saturating_add(poll.frames.len() as u64);
				*health = "streaming".to_string();
				// 最新 UNMotionFrame を memoize (正式 route 用)。
				if let Some(frame) = poll.frames.last() {
					*latest_unmotion_frame = Some(frame.clone());
				}
				for frame in poll.frames {
					pending_unmotion_frames.push_back(frame);
					while pending_unmotion_frames.len() > OUTPUT_PENDING_FRAME_CAPACITY {
						pending_unmotion_frames.pop_front();
					}
				}
				latest_states.insert(poll.stream_id, poll.state);
				status_callback(CoreRuntimeStatusUpdate {
			...
			Ok(MotionFrameStreamWorkerMessage::Error { stream_id, message }) => {
				received = true;
				*health = format!("{} Motion frame stream failed: {message}", stream_id.0);
				tracing::warn!(
					...
					message = %message,
					"Motion frame stream worker error",
				);
			}
			Ok(MotionFrameStreamWorkerMessage::Stopped { .. }) => {
				received = true;
			}
			Err(TryRecvError::Empty) => break,
			Err(TryRecvError::Disconnected) => {
				if !received {
					*health = "Motion frame stream worker disconnected".to_string();
				}
				break;
			}
		}
...
fn runtime_snapshot(
	config: &CoreRuntimeConfig,
	latest_states: &HashMap<StreamId, StreamState>,
	state: RuntimeState,
	output_telemetry: &OutputTelemetry,
) -> RuntimeSnapshot {
...
struct CoreRuntimeWorkers {
	frame_streams: Vec<MotionFrameStreamWorkerHandle>,
	frame_stream_rx: Receiver<MotionFrameStreamWorkerMessage>,
		...
		for stream in &config.frame_streams {
			match open_motion_frame_source(stream) {
				...
				Err(error) => {
					...
					let _ = frame_stream_tx.send(MotionFrameStreamWorkerMessage::Error {
...
mod tests {
	...
	use un_motion_runtime::{MotionFrameStreamPoll, StreamHealth, stream_state_from_unmotion_frame};
	...
	fn modifier_config_maps_smoothing_preset_strings() {
		...
		assert_eq!(modifier_for("ADAPTIVE").smoothing.preset, SmoothingPreset::Adaptive);
		assert_eq!(modifier_for("Medium").smoothing.preset, SmoothingPreset::Medium);
		// unknown value falls back to Off (pass-through) without panicking.
	...
	#[test]
	fn runtime_host_drains_unmotion_poll_into_snapshot() {
		let config = CoreRuntimeConfig {
		...
		frame_stream_tx
			.send(MotionFrameStreamWorkerMessage::Poll(MotionFrameStreamPoll {
		...
		let mut pending_unmotion_frames = VecDeque::new();
		let received = drain_frame_stream_messages(
			&frame_stream_rx,
			&mut latest_states,
			&mut latest_unmotion_frame,
			&mut pending_unmotion_frames,
		...
		assert!(received);
		assert_eq!(pending_unmotion_frames.len(), 1);
	...
	#[test]
	fn neutral_calibration_summary_omits_limb_rotations() {
		let mut samples = BTreeMap::new();
		...
		assert!(
			summary.is_empty(),
			"limb calibration must not create persistent arm/hand offsets: {summary:?}"
		);
	}
