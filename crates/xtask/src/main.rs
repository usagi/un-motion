...
fn mediapipe_pose_fixtures(repo: &Path, raw_args: Vec<OsString>) -> Result<()> {
	...
	for fixture in fixtures {
		...
		let left_thumb_spread = signal_value_from_probe_report(&report, "hand.left.thumb.spread");
		let right_thumb_spread = signal_value_from_probe_report(&report, "hand.right.thumb.spread");
		...
		println!(
			"{} [{}]: yaw={} pitch={} roll={} front={} palmNormalZ=({}, {}) handPalmZ=({}, {}) thumbSpread=({}, {})",
			fixture.file,
			...
			format_optional_f64(left_thumb_spread),
			format_optional_f64(right_thumb_spread)
		);
		...
		check_pose_fixture_signal(&fixture, "hand.left.thumb.spread", left_thumb_spread)?;
		check_pose_fixture_signal(&fixture, "hand.right.thumb.spread", right_thumb_spread)?;
...
struct PoseFixtureSpec {
	...
	#[serde(default)]
	min_left_thumb_spread: Option<f64>,
	#[serde(default)]
	min_right_thumb_spread: Option<f64>,
	#[serde(default)]
	max_left_thumb_spread: Option<f64>,
	#[serde(default)]
	max_right_thumb_spread: Option<f64>,
	#[serde(default)]
	max_abs_left_thumb_spread: Option<f64>,
	#[serde(default)]
	max_abs_right_thumb_spread: Option<f64>,
	#[serde(default)]
...
impl PoseFixtureSpec {
	...
	fn max_abs_for_signal(&self, signal: &str) -> Option<f64> {
		match signal {
			...
			"hand.left.thumb.spread" => self.max_abs_left_thumb_spread,
			"hand.right.thumb.spread" => self.max_abs_right_thumb_spread,
			_ => None,
		}
	}
	...
	fn min_for_signal(&self, signal: &str) -> Option<f64> {
		match signal {
			...
			"hand.left.thumb.spread" => self.min_left_thumb_spread,
			"hand.right.thumb.spread" => self.min_right_thumb_spread,
	...
	fn max_for_signal(&self, signal: &str) -> Option<f64> {
		match signal {
			...
			"hand.left.thumb.spread" => self.max_left_thumb_spread,
			"hand.right.thumb.spread" => self.max_right_thumb_spread,
...
fn unmf(repo: &Path, args: Vec<OsString>) -> Result<()> {
	let Some(subcommand) = args.first() else {
		eprintln!("usage: cargo xtask unmf <stability|noise> [options]");
		bail!("missing unmf subcommand");
	};
	match subcommand.to_string_lossy().as_ref() {
		"stability" | "stability-report" => unmf_stability_report(repo, args[1..].to_vec()),
		"noise" | "noise-report" => unmf_noise_report(repo, args[1..].to_vec()),
		"--help" | "-h" | "help" => {
			eprintln!(
				"usage:\n  cargo xtask unmf stability [--key un-motion/frame] [--topic-mode frame|by-primary-source|by-stream-id] [--duration-ms 5000] [--output report.json] [--min-samples 2]\n  cargo xtask unmf noise --input snapshot.analysis-extras.jsonl|png-dir [--output report.json] [--markdown report.md] [--spectral-points 1024]"
			);
			Ok(())
		}
...
#[serde(rename_all = "camelCase")]
struct UnmfNoiseReport {
	kind: &'static str,
	schema: &'static str,
	source: String,
	sample_count: usize,
	image_count: usize,
	images: UnmfNoiseImageReport,
	landmarks: BTreeMap<String, UnmfNoiseLandmarkReport>,
	rotations: Vec<UnmfNoiseNamedStats>,
	interpretation: Vec<String>,
}

#[derive(Debug, Default, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnmfNoiseImageReport {
	frames: usize,
	width: u32,
	height: u32,
	missing: Vec<String>,
	luma_mean: UnmfNoiseStats,
	temporal_luma_std: UnmfNoiseStats,
	adjacent_mean_abs: UnmfNoiseStats,
	flat_region_mean_abs: UnmfNoiseStats,
	edge_region_mean_abs: UnmfNoiseStats,
	edge_to_flat_ratio_mean: Option<f64>,
	high_frequency_residual_mean_abs: UnmfNoiseStats,
	temporal_spectrum: UnmfNoiseTemporalSpectrumReport,
}

#[derive(Debug, Default, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnmfNoiseTemporalSpectrumReport {
	points: usize,
	spectrum_sample_count: usize,
	sample_rate_hz: f64,
	duration_secs: f64,
	frequency_resolution_hz: f64,
	method: &'static str,
	channels: Vec<UnmfNoiseTemporalSpectrumChannelReport>,
}

#[derive(Debug, Default, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnmfNoiseTemporalSpectrumChannelReport {
	channel: &'static str,
	low_0_to_2_hz: UnmfNoiseStats,
	mid_2_to_10_hz: UnmfNoiseStats,
	high_10_hz_plus: UnmfNoiseStats,
	total_ac_amplitude: UnmfNoiseStats,
	dominant_frequency_hz: UnmfNoiseStats,
}

#[derive(Debug, Default, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnmfNoiseLandmarkReport {
	frames: usize,
	points: usize,
	position_std: UnmfNoiseStats,
	adjacent_delta: UnmfNoiseStats,
	top_std_points: Vec<UnmfNoiseIndexedValue>,
}

#[derive(Debug, Default, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnmfNoiseStats {
	count: usize,
	mean: f64,
	std: f64,
	min: f64,
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct UnmfNoiseNamedStats {
	name: String,
	#[serde(flatten)]
	stats: UnmfNoiseStats,
}

fn unmf_noise_report(repo: &Path, raw_args: Vec<OsString>) -> Result<()> {
	let args = parse_unmf_noise_args(raw_args)?;
	let input = absolutize(repo, &args.input);
	let mut report = if input.is_dir() {
		collect_unmf_noise_from_png_dir(&input, args.spectral_points)?
	} else {
		collect_unmf_noise_from_analysis_extras(&input, args.spectral_points)?
	};
	report.interpretation = interpret_unmf_noise(&report);
	let output = args
		.output
		.map(|path| absolutize(repo, &path))
		.unwrap_or_else(|| default_unmf_noise_output_path(&input));
	if let Some(parent) = output.parent() {
		fs::create_dir_all(parent).with_context(|| format!("failed to create {}", parent.display()))?;
	}
	fs::write(&output, serde_json::to_vec_pretty(&report)?).with_context(|| format!("failed to write {}", output.display()))?;
	let markdown = args
		.markdown
		.map(|path| absolutize(repo, &path))
		.unwrap_or_else(|| output.with_extension("md"));
	fs::write(&markdown, render_unmf_noise_markdown(&report)).with_context(|| format!("failed to write {}", markdown.display()))?;
	println!("wrote {}", output.display());
	println!("wrote {}", markdown.display());
	Ok(())
}

fn parse_unmf_noise_args(raw_args: Vec<OsString>) -> Result<UnmfNoiseArgs> {
	let mut input = None;
	let mut output = None;
	let mut markdown = None;
	let mut spectral_points = 1024usize;
	let mut iter = raw_args.into_iter();
	while let Some(arg) = iter.next() {
		match arg.to_string_lossy().as_ref() {
			"--input" => input = Some(PathBuf::from(next_value(&mut iter, "--input")?)),
			"--output" => output = Some(PathBuf::from(next_value(&mut iter, "--output")?)),
			"--markdown" => markdown = Some(PathBuf::from(next_value(&mut iter, "--markdown")?)),
			"--spectral-points" => {
				spectral_points = next_value(&mut iter, "--spectral-points")?
					.to_string_lossy()
					.parse::<usize>()
					.context("--spectral-points must be a positive integer")?
					.max(1);
			}
			"--help" | "-h" | "help" => {
				eprintln!(
					"usage: cargo xtask unmf noise --input snapshot.analysis-extras.jsonl|png-dir [--output report.json] [--markdown report.md] [--spectral-points 1024]"
				);
				bail!("unmf noise help requested")
			}
			other => bail!("unexpected unmf noise argument: {other}"),
		}
	}
	Ok(UnmfNoiseArgs {
...
fn default_unmf_noise_output_path(input: &Path) -> PathBuf {
	if input.is_dir() {
		input.join("_noise-analysis.json")
	} else {
		input.with_extension("noise-analysis.json")
	}
}

fn collect_unmf_noise_from_png_dir(input: &Path, spectral_points: usize) -> Result<UnmfNoiseReport> {
	let mut image_paths = fs::read_dir(input)?
		.filter_map(|entry| entry.ok().map(|entry| entry.path()))
		.filter(|path| {
			path.extension()
				.and_then(|ext| ext.to_str())
		...
		.filter(|path| !path.file_name().and_then(|name| name.to_str()).unwrap_or_default().starts_with('_'))
		.collect::<Vec<_>>();
	sort_unmf_noise_image_paths(&mut image_paths);
	let image_count = image_paths.len();
	Ok(UnmfNoiseReport {
		kind: "un-motion-noise-analysis",
		schema: "un-motion.dev.noise-analysis.v1",
		source: input.display().to_string(),
		sample_count: 0,
		image_count,
		images: analyze_unmf_noise_images(&image_paths, spectral_points)?,
...
fn collect_unmf_noise_from_analysis_extras(input: &Path, spectral_points: usize) -> Result<UnmfNoiseReport> {
	let mut samples = Vec::new();
	for line in fs::read_to_string(input)?.lines().filter(|line| !line.trim().is_empty()) {
		let entry: JsonValue = serde_json::from_str(line).with_context(|| format!("failed to decode {}", input.display()))?;
		if entry.get("kind").and_then(JsonValue::as_str) != Some("runtime-analysis-extras") {
			continue;
		}
		let Some(values) = entry.pointer("/response/samples").and_then(JsonValue::as_array) else {
			continue;
		};
		samples.extend(
			values
				.iter()
				.filter(|sample| sample.get("kind").and_then(JsonValue::as_str) == Some("mediapipe-analysis-frame"))
				.cloned(),
		);
	}
	let image_paths = resolve_unmf_noise_image_paths(input, &samples)?;
	let image_count = image_paths.len();
	Ok(UnmfNoiseReport {
		kind: "un-motion-noise-analysis",
		schema: "un-motion.dev.noise-analysis.v1",
		...
		image_count,
		images: analyze_unmf_noise_images(&image_paths, spectral_points)?,
		landmarks: analyze_unmf_noise_landmarks(&samples),
		rotations: analyze_unmf_noise_rotations(&samples),
		interpretation: Vec::new(),
	})
}

fn resolve_unmf_noise_image_paths(input: &Path, samples: &[JsonValue]) -> Result<Vec<PathBuf>> {
	let assets_root = input
		.file_name()
		.and_then(|name| name.to_str())
		.and_then(|name| name.strip_suffix(".analysis-extras.jsonl"))
		.and_then(|base| input.parent().map(|parent| parent.join(format!("{base}.analysis-assets"))));
	let mut asset_pngs = Vec::new();
	if let Some(root) = assets_root.as_ref().filter(|root| root.exists()) {
		collect_png_paths_recursive(root, &mut asset_pngs)?;
	}
	let mut out = Vec::new();
	for sample in samples {
		if let Some(path) = sample.pointer("/input/imagePath").and_then(JsonValue::as_str).map(PathBuf::from) {
			if path.exists() {
				out.push(path);
				continue;
			}
			if let Some(file_name) = path.file_name().and_then(|name| name.to_str())
				&& let Some(recovered) = asset_pngs
					.iter()
					.find(|candidate| candidate.file_name().and_then(|name| name.to_str()) == Some(file_name))
			{
				out.push(recovered.clone());
				continue;
			}
		}
		let sequence = sample.pointer("/input/sequence").and_then(JsonValue::as_u64);
		let timestamp = sample.pointer("/input/captureTimestampNs").and_then(JsonValue::as_u64);
		if let (Some(sequence), Some(timestamp)) = (sequence, timestamp) {
			let suffix = format!("seq{sequence}-ts{timestamp}.png");
			if let Some(recovered) = asset_pngs.iter().find(|candidate| {
				candidate
					.file_name()
					.and_then(|name| name.to_str())
...
fn collect_png_paths_recursive(dir: &Path, out: &mut Vec<PathBuf>) -> Result<()> {
	for entry in fs::read_dir(dir).with_context(|| format!("failed to read {}", dir.display()))? {
		let entry = entry?;
		let path = entry.path();
		if path.is_dir() {
			collect_png_paths_recursive(&path, out)?;
		} else if path
			.extension()
			.and_then(|ext| ext.to_str())
			.is_some_and(|ext| ext.eq_ignore_ascii_case("png"))
...
fn analyze_unmf_noise_images(paths: &[PathBuf], spectral_points: usize) -> Result<UnmfNoiseImageReport> {
	let mut missing = Vec::new();
	let mut frames = 0usize;
	let mut width = 0u32;
	let mut height = 0u32;
	let mut luma_mean = Vec::new();
	let mut temporal_mean = Vec::<f64>::new();
	let mut temporal_m2 = Vec::<f64>::new();
	let mut previous_luma: Option<Vec<f64>> = None;
	let mut adjacent = UnmfNoiseAdjacentStats::default();
	let mut high_frequency = Vec::new();
	let mut timestamps_ns = Vec::new();
	let mut spectral_points_xy = Vec::<(u32, u32)>::new();
	let mut spectral_series: [Vec<Vec<f64>>; 3] = std::array::from_fn(|_| Vec::new());
	let high_frequency_stride = paths.len().saturating_div(300).max(1);
	for path in paths {
		if !path.exists() {
		...
		if frames == 0 {
			...
			temporal_mean = vec![0.0; pixel_count];
			temporal_m2 = vec![0.0; pixel_count];
			spectral_points_xy = halton_image_points(spectral_points, width, height);
			spectral_series = std::array::from_fn(|_| vec![Vec::new(); spectral_points_xy.len()]);
		}
		if image.width() != width || image.height() != height {
			missing.push(format!(
				"{}: dimensions changed to {}x{}",
				path.display(),
				image.width(),
				image.height()
			));
		...
		frames += 1;
		let luma = unmf_noise_luma(&image);
		luma_mean.push(mean_f64(&luma));
		let count = frames as f64;
		for (index, value) in luma.iter().copied().enumerate() {
			let delta = value - temporal_mean[index];
			temporal_mean[index] += delta / count;
			temporal_m2[index] += delta * (value - temporal_mean[index]);
		}
		if let Some(previous) = previous_luma.as_ref() {
			adjacent.observe(previous, &luma, width as usize, height as usize);
		}
		previous_luma = Some(luma);
		if frames.saturating_sub(1) % high_frequency_stride == 0 {
			high_frequency.push(unmf_noise_high_frequency_residual(&image));
		}
		if let Some(timestamp) = parse_unmf_noise_timestamp_ns(path) {
			timestamps_ns.push(timestamp);
		}
		for (point_index, (x, y)) in spectral_points_xy.iter().enumerate() {
			let pixel = image.get_pixel(*x, *y).0;
			let (yy, cb, cr) = rgb_to_ycbcr_bt601_full_range(pixel[0] as f64 / 255.0, pixel[1] as f64 / 255.0, pixel[2] as f64 / 255.0);
			spectral_series[0][point_index].push(yy);
			spectral_series[1][point_index].push(cb);
			spectral_series[2][point_index].push(cr);
		}
	}
	if frames == 0 {
		return Ok(UnmfNoiseImageReport {
			missing,
			..UnmfNoiseImageReport::default()
		});
	}
	let temporal_luma_std = temporal_m2.into_iter().map(|m2| (m2 / frames as f64).sqrt()).collect::<Vec<_>>();
	Ok(UnmfNoiseImageReport {
		frames,
		width,
		height,
		missing,
		luma_mean: unmf_noise_stats(luma_mean),
		temporal_luma_std: unmf_noise_stats(temporal_luma_std),
		adjacent_mean_abs: unmf_noise_stats(adjacent.mean_abs),
		flat_region_mean_abs: unmf_noise_stats(adjacent.flat_mean_abs.clone()),
		edge_region_mean_abs: unmf_noise_stats(adjacent.edge_mean_abs.clone()),
		edge_to_flat_ratio_mean: ratio_f64(mean_f64(&adjacent.edge_mean_abs), mean_f64(&adjacent.flat_mean_abs)),
		high_frequency_residual_mean_abs: unmf_noise_stats(high_frequency),
		temporal_spectrum: analyze_unmf_noise_temporal_spectrum_from_series(spectral_series, frames, &timestamps_ns),
	})
}

fn unmf_noise_luma(image: &image::RgbImage) -> Vec<f64> {
	image
		.pixels()
		.map(|pixel| {
			let [r, g, b] = pixel.0;
...
fn unmf_noise_temporal_std(frames: &[(u32, u32, Vec<f64>)]) -> Vec<f64> {
	let Some((_, _, first)) = frames.first() else {
		return Vec::new();
	};
	...
	for index in 0..first.len() {
		let series = frames
			.iter()
			.filter_map(|(_, _, luma)| luma.get(index).copied())
			.collect::<Vec<_>>();
		out.push(std_dev_f64(&series, mean_f64(&series)));
	}
	out
}

#[derive(Default)]
struct UnmfNoiseAdjacentStats {
	mean_abs: Vec<f64>,
	flat_mean_abs: Vec<f64>,
	edge_mean_abs: Vec<f64>,
}

impl UnmfNoiseAdjacentStats {
	fn observe(&mut self, previous: &[f64], current: &[f64], width: usize, height: usize) {
		let edge = unmf_noise_edge_mask(previous, width, height);
		let mut all_sum = 0.0;
		...
		for (index, (a, b)) in previous.iter().zip(current).enumerate() {
			let diff = (b - a).abs();
			all_sum += diff;
			all_count += 1;
			if edge.get(index).copied().unwrap_or(false) {
				edge_sum += diff;
				edge_count += 1;
			} else {
				flat_sum += diff;
		...
		self.mean_abs.push(if all_count == 0 { 0.0 } else { all_sum / all_count as f64 });
		self.edge_mean_abs
			.push(if edge_count == 0 { 0.0 } else { edge_sum / edge_count as f64 });
		self.flat_mean_abs
			.push(if flat_count == 0 { 0.0 } else { flat_sum / flat_count as f64 });
	}
}

fn unmf_noise_adjacent_stats(frames: &[(u32, u32, Vec<f64>)]) -> UnmfNoiseAdjacentStats {
	let mut out = UnmfNoiseAdjacentStats::default();
	for pair in frames.windows(2) {
		let (width, height, previous) = &pair[0];
		let (_, _, current) = &pair[1];
		let edge = unmf_noise_edge_mask(previous, *width as usize, *height as usize);
		let mut all = Vec::with_capacity(previous.len());
		...
		for (index, (a, b)) in previous.iter().zip(current).enumerate() {
			...
			all.push(diff);
			if edge.get(index).copied().unwrap_or(false) {
				edge_values.push(diff);
			} else {
				flat_values.push(diff);
			}
		}
		out.mean_abs.push(mean_f64(&all));
		out.edge_mean_abs.push(mean_f64(&edge_values));
		out.flat_mean_abs.push(mean_f64(&flat_values));
	}
	out
}

fn unmf_noise_edge_mask(luma: &[f64], width: usize, height: usize) -> Vec<bool> {
	if width < 3 || height < 3 || luma.len() < width.saturating_mul(height) {
		return vec![false; luma.len()];
	}
	let mut magnitude = vec![0.0; luma.len()];
	for y in 1..height - 1 {
		for x in 1..width - 1 {
			let index = y * width + x;
			let gx = luma[index + 1] - luma[index - 1];
			let gy = luma[index + width] - luma[index - width];
			magnitude[index] = (gx * gx + gy * gy).sqrt();
		}
	}
	let threshold = percentile_f64(magnitude.clone(), 0.75);
	magnitude.into_iter().map(|value| value >= threshold).collect()
}

fn unmf_noise_high_frequency_residual(image: &image::RgbImage) -> f64 {
	let blurred = image::imageops::blur(image, 1.0);
	let mut sum = 0.0;
	let mut count = 0;
	for (a, b) in image.pixels().zip(blurred.pixels()) {
		for channel in 0..3 {
			sum += ((a.0[channel] as f64 - b.0[channel] as f64) / 255.0).abs();
			count += 1;
		}
	}
...
fn parse_unmf_noise_timestamp_ns(path: &Path) -> Option<u64> {
	let name = path.file_name()?.to_str()?;
	let start = name.find("-ts")? + 3;
	let digits = name[start..].chars().take_while(|ch| ch.is_ascii_digit()).collect::<String>();
	digits.parse().ok()
}

fn sort_unmf_noise_image_paths(paths: &mut [PathBuf]) {
	paths.sort_by(|left, right| {
		parse_unmf_noise_timestamp_ns(left)
			.cmp(&parse_unmf_noise_timestamp_ns(right))
			.then_with(|| left.cmp(right))
	});
}

fn analyze_unmf_noise_temporal_spectrum(
	frames: &[image::RgbImage],
	timestamps_ns: &[u64],
	requested_points: usize,
) -> UnmfNoiseTemporalSpectrumReport {
	let Some(first) = frames.first() else {
		return UnmfNoiseTemporalSpectrumReport::default();
	};
	if frames.len() < 4 || first.width() == 0 || first.height() == 0 {
		return UnmfNoiseTemporalSpectrumReport::default();
	}
	let sample_rate_hz = estimate_unmf_noise_sample_rate_hz(frames.len(), timestamps_ns);
	let duration_secs = frames.len() as f64 / sample_rate_hz.max(1e-9);
	let spectrum_sample_count = if frames.len() > 256 {
		largest_power_of_two_at_most(frames.len())
	} else {
		frames.len()
	};
	let frequency_resolution_hz = sample_rate_hz / spectrum_sample_count as f64;
	let points = halton_image_points(requested_points, first.width(), first.height());
	let mut channels = unmf_noise_spectrum_channel_accumulators();
	for (x, y) in &points {
		let series = unmf_noise_pixel_series(frames, *x, *y);
		for (index, values) in series.iter().enumerate() {
			channels[index].push_spectrum(values, sample_rate_hz);
		}
	}
	UnmfNoiseTemporalSpectrumReport {
		points: points.len(),
		...
		method: "Halton quasi-random points; per-point mean-removed DFT for short series or prefix radix-2 FFT for long series; YCbCr is full-range BT.601 from gamma-coded sRGB",
		channels: channels.into_iter().map(UnmfNoiseSpectrumChannelAccumulator::finish).collect(),
	}
}

fn analyze_unmf_noise_temporal_spectrum_from_series(
	series: [Vec<Vec<f64>>; 3],
	frame_count: usize,
	timestamps_ns: &[u64],
) -> UnmfNoiseTemporalSpectrumReport {
	if frame_count < 4 || series[0].is_empty() {
		return UnmfNoiseTemporalSpectrumReport::default();
	}
	let sample_rate_hz = estimate_unmf_noise_sample_rate_hz(frame_count, timestamps_ns);
	let duration_secs = frame_count as f64 / sample_rate_hz.max(1e-9);
	let spectrum_sample_count = if frame_count > 256 {
		largest_power_of_two_at_most(frame_count)
	} else {
		frame_count
	};
	let frequency_resolution_hz = sample_rate_hz / spectrum_sample_count as f64;
	...
	for channel_index in 0..3 {
		for values in &series[channel_index] {
			channels[channel_index].push_spectrum(values, sample_rate_hz);
		}
	}
	UnmfNoiseTemporalSpectrumReport {
		points: series[0].len(),
		...
		duration_secs,
		frequency_resolution_hz,
		method: "Halton quasi-random points; streaming image pass; per-point mean-removed DFT for short series or prefix radix-2 FFT for long series; YCbCr is full-range BT.601 from gamma-coded sRGB",
		channels: channels.into_iter().map(UnmfNoiseSpectrumChannelAccumulator::finish).collect(),
	}
}

fn estimate_unmf_noise_sample_rate_hz(frame_count: usize, timestamps_ns: &[u64]) -> f64 {
	if timestamps_ns.len() >= 2
		&& let (Some(first), Some(last)) = (timestamps_ns.iter().min(), timestamps_ns.iter().max())
	{
		let duration_secs = last.saturating_sub(*first) as f64 / 1_000_000_000.0;
		if duration_secs > 0.0 {
			return (timestamps_ns.len().saturating_sub(1)) as f64 / duration_secs;
		}
	}
	frame_count as f64
}

fn halton_image_points(count: usize, width: u32, height: u32) -> Vec<(u32, u32)> {
	let width = width.max(1);
	let height = height.max(1);
	let mut points = Vec::with_capacity(count);
	for index in 1..=count {
		let x = (halton(index as u64, 2) * width as f64).floor().clamp(0.0, (width - 1) as f64) as u32;
		let y = (halton(index as u64, 3) * height as f64).floor().clamp(0.0, (height - 1) as f64) as u32;
		points.push((x, y));
	}
	points
}
...
fn unmf_noise_pixel_series(frames: &[image::RgbImage], x: u32, y: u32) -> [Vec<f64>; 3] {
	let mut y_values = Vec::with_capacity(frames.len());
	let mut cb_values = Vec::with_capacity(frames.len());
	let mut cr_values = Vec::with_capacity(frames.len());
	for frame in frames {
		let pixel = frame.get_pixel(x.min(frame.width() - 1), y.min(frame.height() - 1)).0;
		let rr = pixel[0] as f64 / 255.0;
		let gg = pixel[1] as f64 / 255.0;
		let bb = pixel[2] as f64 / 255.0;
		let (y, cb, cr) = rgb_to_ycbcr_bt601_full_range(rr, gg, bb);
		y_values.push(y);
		cb_values.push(cb);
		cr_values.push(cr);
	}
...
fn rgb_to_ycbcr_bt601_full_range(r: f64, g: f64, b: f64) -> (f64, f64, f64) {
	let y = 0.299 * r + 0.587 * g + 0.114 * b;
	let cb = (b - y) / (2.0 * (1.0 - 0.114));
	let cr = (r - y) / (2.0 * (1.0 - 0.299));
	(y, cb, cr)
}

struct UnmfNoiseSpectrumChannelAccumulator {
	channel: &'static str,
	low: Vec<f64>,
	mid: Vec<f64>,
	high: Vec<f64>,
...
impl UnmfNoiseSpectrumChannelAccumulator {
	fn push_spectrum(&mut self, values: &[f64], sample_rate_hz: f64) {
		let spectrum = dft_band_amplitudes(values, sample_rate_hz);
		self.low.push(spectrum.low);
		self.mid.push(spectrum.mid);
		self.high.push(spectrum.high);
		self.total.push(spectrum.total);
		self.dominant.push(spectrum.dominant_frequency_hz);
	}

	fn finish(self) -> UnmfNoiseTemporalSpectrumChannelReport {
		UnmfNoiseTemporalSpectrumChannelReport {
			channel: self.channel,
			low_0_to_2_hz: unmf_noise_stats(self.low),
			mid_2_to_10_hz: unmf_noise_stats(self.mid),
			high_10_hz_plus: unmf_noise_stats(self.high),
			total_ac_amplitude: unmf_noise_stats(self.total),
			dominant_frequency_hz: unmf_noise_stats(self.dominant),
		}
	}
}

fn unmf_noise_spectrum_channel_accumulators() -> [UnmfNoiseSpectrumChannelAccumulator; 3] {
	["y", "cb", "cr"].map(|channel| UnmfNoiseSpectrumChannelAccumulator {
		channel,
		low: Vec::new(),
		mid: Vec::new(),
		high: Vec::new(),
...
fn dft_band_amplitudes(values: &[f64], sample_rate_hz: f64) -> DftBandAmplitudes {
	let n = values.len();
	if n < 4 || sample_rate_hz <= 0.0 {
		return DftBandAmplitudes::default();
	}
	if n > 256 {
		return fft_band_amplitudes(values, sample_rate_hz);
	}
	...
	for bin in 1..=n / 2 {
		...
		for (sample, value) in values.iter().enumerate() {
			let phase = -2.0 * std::f64::consts::PI * bin as f64 * sample as f64 / n as f64;
			let centered = value - mean;
			real += centered * phase.cos();
			imag += centered * phase.sin();
		}
		let amplitude = (real * real + imag * imag).sqrt() * 2.0 / n as f64;
	...
	DftBandAmplitudes {
		low: low_power.sqrt(),
		mid: mid_power.sqrt(),
		high: high_power.sqrt(),
		total: total_power.sqrt(),
...
fn fft_band_amplitudes(values: &[f64], sample_rate_hz: f64) -> DftBandAmplitudes {
	let n = largest_power_of_two_at_most(values.len());
	if n < 4 {
		return DftBandAmplitudes::default();
	}
	let mean = mean_f64(&values[..n]);
	let mut data = values[..n]
		.iter()
		.map(|value| Complex64 { re: value - mean, im: 0.0 })
		.collect::<Vec<_>>();
	radix2_fft(&mut data);
	band_amplitudes_from_fft(&data, sample_rate_hz)
}
...
fn radix2_fft(data: &mut [Complex64]) {
	let n = data.len();
	let mut j = 0usize;
	for i in 1..n {
		let mut bit = n >> 1;
...
fn band_amplitudes_from_fft(data: &[Complex64], sample_rate_hz: f64) -> DftBandAmplitudes {
	let n = data.len();
	let mut low_power = 0.0;
	let mut mid_power = 0.0;
	let mut high_power = 0.0;
	...
	let mut dominant_power = 0.0;
	let mut dominant_frequency_hz = 0.0;
	for (bin, value) in data.iter().enumerate().take(n / 2 + 1).skip(1) {
		let frequency_hz = bin as f64 * sample_rate_hz / n as f64;
		let amplitude = (value.re * value.re + value.im * value.im).sqrt() * 2.0 / n as f64;
		let power = amplitude * amplitude;
		total_power += power;
		if frequency_hz <= 2.0 {
			low_power += power;
...
fn analyze_unmf_noise_landmarks(samples: &[JsonValue]) -> BTreeMap<String, UnmfNoiseLandmarkReport> {
	let mut groups: BTreeMap<String, Vec<Vec<[f64; 3]>>> = BTreeMap::new();
	for sample in samples {
		collect_noise_landmarks(&mut groups, "pose", sample.pointer("/native/pose/landmarks"));
		collect_noise_landmarks(&mut groups, "poseWorld", sample.pointer("/native/pose/worldLandmarks"));
		collect_noise_landmarks(&mut groups, "face", sample.pointer("/native/face/landmarks"));
		if let Some(hands) = sample.pointer("/native/hands/hands").and_then(JsonValue::as_array) {
			for (index, hand) in hands.iter().enumerate() {
				collect_noise_landmarks(&mut groups, &format!("hand{index}"), hand.get("landmarks"));
				collect_noise_landmarks(&mut groups, &format!("hand{index}World"), hand.get("worldLandmarks"));
			}
		}
		for name in [
			"poseLandmarks",
			...
			collect_noise_landmarks(
				&mut groups,
				&format!("holistic.{name}"),
				sample.pointer(&format!("/native/holistic/{name}")),
			);
		}
	}
	groups
		.into_iter()
		.filter_map(|(name, frames)| unmf_noise_landmark_report(frames).map(|report| (name, report)))
		.collect()
}

fn collect_noise_landmarks(groups: &mut BTreeMap<String, Vec<Vec<[f64; 3]>>>, name: &str, value: Option<&JsonValue>) {
	let Some(points) = value.and_then(JsonValue::as_array) else {
		return;
	};
	let frame = points
		.iter()
		...
		.collect::<Vec<_>>();
	if !frame.is_empty() {
		groups.entry(name.to_string()).or_default().push(frame);
	}
}

fn unmf_noise_landmark_report(frames: Vec<Vec<[f64; 3]>>) -> Option<UnmfNoiseLandmarkReport> {
	if frames.len() < 2 {
		return None;
	}
	let points = frames.iter().map(Vec::len).min()?;
	let mut point_std = Vec::with_capacity(points);
	for point in 0..points {
		let xs = frames.iter().map(|frame| frame[point][0]).collect::<Vec<_>>();
		let ys = frames.iter().map(|frame| frame[point][1]).collect::<Vec<_>>();
		let zs = frames.iter().map(|frame| frame[point][2]).collect::<Vec<_>>();
		let sx = std_dev_f64(&xs, mean_f64(&xs));
		let sy = std_dev_f64(&ys, mean_f64(&ys));
		let sz = std_dev_f64(&zs, mean_f64(&zs));
		point_std.push((sx * sx + sy * sy + sz * sz).sqrt());
	}
	let mut deltas = Vec::new();
	for pair in frames.windows(2) {
		for point in 0..points {
			deltas.push(distance3(pair[0][point], pair[1][point]));
		}
	}
	let mut ranked = point_std.iter().copied().enumerate().collect::<Vec<_>>();
	ranked.sort_by(|a, b| b.1.total_cmp(&a.1));
	Some(UnmfNoiseLandmarkReport {
		frames: frames.len(),
		points,
		position_std: unmf_noise_stats(point_std),
		adjacent_delta: unmf_noise_stats(deltas),
		top_std_points: ranked
			.into_iter()
			.take(12)
			.map(|(index, value)| UnmfNoiseIndexedValue { index, value })
			.collect(),
	})
}

fn analyze_unmf_noise_rotations(samples: &[JsonValue]) -> Vec<UnmfNoiseNamedStats> {
	let mut channels: BTreeMap<String, Vec<[f64; 4]>> = BTreeMap::new();
	for sample in samples {
		if let Some(frame) = sample.get("unmotionFrame") {
			collect_noise_rotations("", frame, &mut channels);
		}
	}
	let mut out = Vec::new();
	for (name, quats) in channels {
		if quats.len() < 2 {
			continue;
		}
		let mut deltas = Vec::new();
		let mut previous = normalize_quat4(quats[0]);
		for quat in quats.into_iter().skip(1) {
			let mut current = normalize_quat4(quat);
			if quat_dot4(previous, current) < 0.0 {
				current = [-current[0], -current[1], -current[2], -current[3]];
			}
			deltas.push(quat_delta_deg(previous, current));
			previous = current;
		}
		out.push(UnmfNoiseNamedStats {
			name,
			stats: unmf_noise_stats(deltas),
		});
	}
	out.sort_by(|a, b| b.stats.p95.total_cmp(&a.stats.p95));
	out.truncate(24);
	out
}

fn collect_noise_rotations(prefix: &str, value: &JsonValue, out: &mut BTreeMap<String, Vec<[f64; 4]>>) {
	match value {
		JsonValue::Object(map) => {
			for (key, child) in map {
				...
				if key == "rotation" {
					if let Some(quat) = json_quat4(child) {
						out.entry(prefix.to_string()).or_default().push(quat);
					}
				} else {
					collect_noise_rotations(&path, child, out);
				}
			}
		}
		JsonValue::Array(items) => {
			for (index, child) in items.iter().enumerate() {
				collect_noise_rotations(&format!("{prefix}[{index}]"), child, out);
			}
		}
		_ => {}
	}
...
fn json_quat4(value: &JsonValue) -> Option<[f64; 4]> {
...
fn quat_dot4(a: [f64; 4], b: [f64; 4]) -> f64 {
	a[0] * b[0] + a[1] * b[1] + a[2] * b[2] + a[3] * b[3]
}

fn quat_delta_deg(a: [f64; 4], b: [f64; 4]) -> f64 {
	let dot = quat_dot4(a, b).abs().clamp(-1.0, 1.0);
...
fn unmf_noise_stats(mut values: Vec<f64>) -> UnmfNoiseStats {
	values.retain(|value| value.is_finite());
	if values.is_empty() {
		return UnmfNoiseStats::default();
	}
...
fn percentile_f64(mut values: Vec<f64>, p: f64) -> f64 {
	values.retain(|value| value.is_finite());
	values.sort_by(f64::total_cmp);
	percentile_sorted_f64(&values, p)
}

fn percentile_sorted_f64(values: &[f64], p: f64) -> f64 {
	if values.is_empty() {
		return 0.0;
	}
...
fn ratio_f64(a: f64, b: f64) -> Option<f64> {
	(b.abs() > 1e-12).then_some(a / b)
}

fn interpret_unmf_noise(report: &UnmfNoiseReport) -> Vec<String> {
	let mut notes = Vec::new();
	if let Some(ratio) = report.images.edge_to_flat_ratio_mean {
		if ratio > 2.0 {
			notes.push(
				"Adjacent image changes are edge-dominant; silhouette or texture shimmer is likely feeding landmark jitter.".to_string(),
			);
		} else {
			notes.push(
				"Adjacent image changes are not strongly edge-dominant; sensor, codec, or global luminance noise should be considered."
					.to_string(),
			);
		}
	}
	if report.images.high_frequency_residual_mean_abs.mean > 0.015 {
		notes.push("High-frequency residual is visible enough that a small pre-MediaPipe spatial denoise should be tested.".to_string());
	}
	if !report.landmarks.is_empty() {
		let mut top = report
			.landmarks
			.iter()
			.map(|(name, item)| (name.as_str(), item.adjacent_delta.p95))
			.collect::<Vec<_>>();
		top.sort_by(|a, b| b.1.total_cmp(&a.1));
		notes.push(format!(
			"Largest raw landmark p95 deltas: {}",
			top.into_iter()
				.take(5)
	...
	if !report.rotations.is_empty() {
		notes.push(format!(
			"Largest final rotation p95 deltas: {}",
			report
				.rotations
				.iter()
				.take(5)
				.map(|item| format!("{}={:.3}deg", item.name, item.stats.p95))
				.collect::<Vec<_>>()
				.join(", ")
		));
	}
...
fn render_unmf_noise_markdown(report: &UnmfNoiseReport) -> String {
	let mut out = String::new();
	out.push_str("# UNMotion Noise Analysis\n\n");
	out.push_str(&format!("- Source: `{}`\n", report.source));
	out.push_str(&format!("- Samples: {}\n", report.sample_count));
	out.push_str(&format!("- Images: {}\n\n", report.image_count));
	out.push_str("## Images\n");
	out.push_str(&format!("- lumaMean: {}\n", compact_unmf_noise_stats(&report.images.luma_mean)));
	out.push_str(&format!(
		"- temporalLumaStd: {}\n",
		compact_unmf_noise_stats(&report.images.temporal_luma_std)
	));
	out.push_str(&format!(
		"- adjacentMeanAbs: {}\n",
		compact_unmf_noise_stats(&report.images.adjacent_mean_abs)
	));
	out.push_str(&format!(
		"- flatRegionMeanAbs: {}\n",
		compact_unmf_noise_stats(&report.images.flat_region_mean_abs)
	));
	out.push_str(&format!(
		"- edgeRegionMeanAbs: {}\n",
		compact_unmf_noise_stats(&report.images.edge_region_mean_abs)
	));
	out.push_str(&format!(
		"- highFrequencyResidualMeanAbs: {}\n",
		compact_unmf_noise_stats(&report.images.high_frequency_residual_mean_abs)
	));
	out.push_str(&format!("- edgeToFlatRatioMean: {:?}\n\n", report.images.edge_to_flat_ratio_mean));
	if report.images.temporal_spectrum.points > 0 {
		out.push_str("## Temporal Spectrum\n");
		out.push_str(&format!(
			"- method: {}\n- points: {}\n- spectrumSampleCount: {}\n- sampleRateHz: {:.3}\n- durationSecs: {:.3}\n- frequencyResolutionHz: {:.3}\n",
			report.images.temporal_spectrum.method,
			report.images.temporal_spectrum.points,
			report.images.temporal_spectrum.spectrum_sample_count,
			report.images.temporal_spectrum.sample_rate_hz,
			report.images.temporal_spectrum.duration_secs,
			report.images.temporal_spectrum.frequency_resolution_hz,
		));
		out.push_str(
			"| channel | low 0-2Hz mean/p95 | mid 2-10Hz mean/p95 | high 10Hz+ mean/p95 | total AC mean/p95 | dominant Hz p50/p95 |\n",
		);
		out.push_str("|---|---:|---:|---:|---:|---:|\n");
		for channel in &report.images.temporal_spectrum.channels {
			out.push_str(&format!(
				"| {} | {:.6}/{:.6} | {:.6}/{:.6} | {:.6}/{:.6} | {:.6}/{:.6} | {:.3}/{:.3} |\n",
				channel.channel,
				channel.low_0_to_2_hz.mean,
				channel.low_0_to_2_hz.p95,
				channel.mid_2_to_10_hz.mean,
				channel.mid_2_to_10_hz.p95,
				channel.high_10_hz_plus.mean,
				channel.high_10_hz_plus.p95,
				channel.total_ac_amplitude.mean,
				channel.total_ac_amplitude.p95,
				channel.dominant_frequency_hz.p50,
				channel.dominant_frequency_hz.p95,
			));
		}
		out.push('\n');
	}
	...
	if !report.rotations.is_empty() {
		out.push_str("\n## Top Rotation Deltas\n");
		for item in report.rotations.iter().take(12) {
			out.push_str(&format!(
				"- {}: p95={:.3}deg max={:.3}deg\n",
				item.name, item.stats.p95, item.stats.max
			));
		}
	}
	out
...
fn compact_unmf_noise_stats(stats: &UnmfNoiseStats) -> String {
	format!(
		"count={} mean={:.6} std={:.6} p50={:.6} p95={:.6} max={:.6}",
		stats.count, stats.mean, stats.std, stats.p50, stats.p95, stats.max
	)
}
