...
pub fn vmc_frame_pose_for_unmotion_frame(frame: &UNMotionFrame) -> VmcFramePose {
	vmc_frame_pose_for_unmotion_frame_with_options(frame, default_blendshape_map_ref(), ChestStabilizationOptions::disabled())
}

pub fn vmc_signal_pose_for_unmotion_frame(frame: &UNMotionFrame) -> VmcFramePose {
	vmc_signal_pose_for_unmotion_frame_with_options(frame, default_blendshape_map_ref(), ChestStabilizationOptions::disabled())
}
...
pub fn vmc_packets_for_frame(frame: &UNMotionFrame) -> Vec<OscPacket> {
	vmc_packets_for_frame_with_options(frame, true, default_blendshape_map_ref(), ChestStabilizationOptions::disabled())
}

pub fn vmc_packets_for_frame_without_ok(frame: &UNMotionFrame) -> Vec<OscPacket> {
	vmc_packets_for_frame_with_options(frame, false, default_blendshape_map_ref(), ChestStabilizationOptions::disabled())
}
...
fn vmc_packets_for_frame_with_options(
	...
	let mut packets = Vec::with_capacity(vmc_packet_capacity_hint(frame, send_ok_packet));
	if send_ok_packet {
...
fn vmc_packet_capacity_hint(frame: &UNMotionFrame, send_ok_packet: bool) -> usize {
	let ok = usize::from(send_ok_packet);
	let face = frame.face.as_ref().map_or(0, |face| face.expressions.len().saturating_add(1));
	let body = if frame_has_direct_humanoid_pose(frame) {
		frame.body.as_ref().and_then(|body| body.humanoid.as_ref()).map_or(0, |humanoid| {
			humanoid.bones.len().saturating_add(usize::from(humanoid.root.is_some()))
		})
	} else if frame_has_hand_motion(frame) {
		1 + frame.left_hand.as_ref().map_or(0, hand_packet_capacity_hint) + frame.right_hand.as_ref().map_or(0, hand_packet_capacity_hint)
	} else {
		0
	};
	ok + face + body
...
fn hand_packet_capacity_hint(hand: &un_motion_frame::HandMotion) -> usize {
	usize::from(hand.wrist.is_some()) + hand.fingers.iter().map(|finger| finger.joints.len()).sum::<usize>()
}
...
fn direct_humanoid_pose_packets(frame: &UNMotionFrame, chest_stabilization: ChestStabilizationOptions) -> Vec<OscPacket> {
	...
	let mut packets = Vec::with_capacity(1 + humanoid.bones.len() + hand_packet_capacity_hint_for_frame(frame));
...
fn hand_motion_transforms(frame: &UNMotionFrame) -> Vec<VmcPoseTransform> {
	let mut out = Vec::with_capacity(hand_packet_capacity_hint_for_frame(frame));
...
fn hand_motion_transforms_for_side(side: BodySide, hand: &HandMotion) -> Vec<VmcPoseTransform> {
	let mut out = Vec::with_capacity(hand_packet_capacity_hint(hand));
	...
	for finger in &hand.fingers {
		for (index, joint) in finger.joints.iter().enumerate() {
			let Some(name) = finger_bone_name(side, finger.finger, index) else {
				continue;
			};
...
fn hand_packet_capacity_hint_for_frame(frame: &UNMotionFrame) -> usize {
	frame.left_hand.as_ref().map_or(0, hand_packet_capacity_hint) + frame.right_hand.as_ref().map_or(0, hand_packet_capacity_hint)
}

fn finger_bone_name(side: BodySide, finger: Finger, index: usize) -> Option<&'static str> {
	match (side, finger, index) {
		(BodySide::Left, Finger::Thumb, 0) => Some("LeftThumbProximal"),
		(BodySide::Left, Finger::Thumb, 1) => Some("LeftThumbIntermediate"),
		(BodySide::Left, Finger::Thumb, 2) => Some("LeftThumbDistal"),
		(BodySide::Left, Finger::Index, 0) => Some("LeftIndexProximal"),
		(BodySide::Left, Finger::Index, 1) => Some("LeftIndexIntermediate"),
		(BodySide::Left, Finger::Index, 2) => Some("LeftIndexDistal"),
		(BodySide::Left, Finger::Middle, 0) => Some("LeftMiddleProximal"),
		(BodySide::Left, Finger::Middle, 1) => Some("LeftMiddleIntermediate"),
		(BodySide::Left, Finger::Middle, 2) => Some("LeftMiddleDistal"),
		(BodySide::Left, Finger::Ring, 0) => Some("LeftRingProximal"),
		(BodySide::Left, Finger::Ring, 1) => Some("LeftRingIntermediate"),
		(BodySide::Left, Finger::Ring, 2) => Some("LeftRingDistal"),
		(BodySide::Left, Finger::Little, 0) => Some("LeftLittleProximal"),
		(BodySide::Left, Finger::Little, 1) => Some("LeftLittleIntermediate"),
		(BodySide::Left, Finger::Little, 2) => Some("LeftLittleDistal"),
		(BodySide::Right, Finger::Thumb, 0) => Some("RightThumbProximal"),
		(BodySide::Right, Finger::Thumb, 1) => Some("RightThumbIntermediate"),
		(BodySide::Right, Finger::Thumb, 2) => Some("RightThumbDistal"),
		(BodySide::Right, Finger::Index, 0) => Some("RightIndexProximal"),
		(BodySide::Right, Finger::Index, 1) => Some("RightIndexIntermediate"),
		(BodySide::Right, Finger::Index, 2) => Some("RightIndexDistal"),
		(BodySide::Right, Finger::Middle, 0) => Some("RightMiddleProximal"),
		(BodySide::Right, Finger::Middle, 1) => Some("RightMiddleIntermediate"),
		(BodySide::Right, Finger::Middle, 2) => Some("RightMiddleDistal"),
		(BodySide::Right, Finger::Ring, 0) => Some("RightRingProximal"),
		(BodySide::Right, Finger::Ring, 1) => Some("RightRingIntermediate"),
		(BodySide::Right, Finger::Ring, 2) => Some("RightRingDistal"),
		(BodySide::Right, Finger::Little, 0) => Some("RightLittleProximal"),
		(BodySide::Right, Finger::Little, 1) => Some("RightLittleIntermediate"),
		(BodySide::Right, Finger::Little, 2) => Some("RightLittleDistal"),
		_ => None,
	}
}

fn rest_position_for_bone(name: &str) -> [f32; 3] {
	rest_position_map_ref().get(name).copied().unwrap_or([0.0, 0.0, 0.0])
}

fn rest_position_map_ref() -> &'static HashMap<&'static str, [f32; 3]> {
	static REST_POSITION_MAP: OnceLock<HashMap<&'static str, [f32; 3]>> = OnceLock::new();
	REST_POSITION_MAP.get_or_init(|| HUMANOID_BONE_REST_POSES.iter().map(|rest| (rest.name, rest.position)).collect())
}
...
fn hand_rotation(frame: &UNMotionFrame, side: BodySide) -> [f32; 4] {
	...
	let hand_prefix = signal_prefix2("hand", prefix);
	let palm_prefix = signal_prefix(&hand_prefix, "palm");
	let wrist_prefix = signal_prefix(&hand_prefix, "wrist");
	let roll = scalar_signal_by_prefix_suffix(frame, &palm_prefix, ".roll").unwrap_or(0.0);
	let z = scalar_signal_by_prefix_suffix(frame, &wrist_prefix, ".z").unwrap_or(0.0);
...
fn forward_stop_palm_pose(frame: &UNMotionFrame, side: BodySide) -> bool {
	...
	let hand_prefix = signal_prefix2("hand", prefix);
	let palm_prefix = signal_prefix(&hand_prefix, "palm");
	let Some(forward) = signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "forward"), frame) else {
		return false;
	};
	let Some(across) = signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "across"), frame) else {
		return false;
	};
...
fn hand_palm_basis_rotation(frame: &UNMotionFrame, side: BodySide) -> Option<[f32; 4]> {
	let prefix = side.signal_prefix();
	let palm_prefix = hand_palm_signal_prefix(prefix);
	let forward = signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "forward"), frame)?;
	let normal = signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "normal"), frame)
		.or_else(|| signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "across"), frame))?;
...
fn overhead_lower_arm_direction(frame: &UNMotionFrame, side: BodySide, lower: [f32; 3]) -> [f32; 3] {
	let palm_prefix = hand_palm_signal_prefix(side.signal_prefix());
	if !hands_are_close_overhead(frame) || signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "across"), frame).is_none() {
		return lower;
	}
...
fn close_front_prayer_pose(frame: &UNMotionFrame) -> bool {
	...
	for side in [BodySide::Left, BodySide::Right] {
		...
		if let (Some(forward), Some(across)) = (
			signal_vec3_by_prefix(&signal_prefix(&hand_palm_signal_prefix(prefix), "forward"), frame),
			signal_vec3_by_prefix(&signal_prefix(&hand_palm_signal_prefix(prefix), "across"), frame),
		) {
...
fn arm_plane_secondary(frame: &UNMotionFrame, side: BodySide, role: ArmSegmentRole) -> Option<[f32; 3]> {
	...
	if role == ArmSegmentRole::Upper
		&& let Some(normal) = signal_vec3_by_prefix(&signal_prefix(&palm_prefix, "normal"), frame)
...
fn arm_point(frame: &UNMotionFrame, prefix: &str, joint: &str) -> Option<[f32; 3]> {
	point_from_signal_prefix(frame, &signal_prefix(&signal_prefix2("arm", prefix), joint))
}

fn torso_point(frame: &UNMotionFrame, prefix: &str, joint: &str) -> Option<[f32; 3]> {
	point_from_signal_prefix(frame, &signal_prefix(&signal_prefix2("torso", prefix), joint))
}
...
fn leg_point(frame: &UNMotionFrame, prefix: &str, joint: &str) -> Option<[f32; 3]> {
	point_from_signal_prefix(frame, &signal_prefix(&signal_prefix2("leg", prefix), joint))
}
...
fn foot_point(frame: &UNMotionFrame, prefix: &str, joint: &str) -> Option<[f32; 3]> {
	point_from_signal_prefix(frame, &signal_prefix(&signal_prefix2("foot", prefix), joint))
}

fn signal_vec3(frame: &UNMotionFrame, prefix: &str) -> Option<[f32; 3]> {
	signal_vec3_by_prefix(prefix, frame)
}

fn signal_vec3_by_prefix(prefix: &str, frame: &UNMotionFrame) -> Option<[f32; 3]> {
	Some([
		scalar_signal_by_prefix_suffix(frame, prefix, ".x")?,
		scalar_signal_by_prefix_suffix(frame, prefix, ".y")?,
		scalar_signal_by_prefix_suffix(frame, prefix, ".z")?,
	])
}

fn point_from_signal_prefix(frame: &UNMotionFrame, prefix: &str) -> Option<[f32; 3]> {
	Some([
		scalar_signal_by_prefix_suffix(frame, prefix, ".x")?,
		scalar_signal_by_prefix_suffix(frame, prefix, ".y")?,
		scalar_signal_by_prefix_suffix(frame, prefix, ".z").unwrap_or(0.0),
	])
}
...
fn hand_finger_fold(frame: &UNMotionFrame, prefix: &str) -> f32 {
	...
	for finger in ["middle", "ring", "little"] {
		if let Some(value) = scalar_signal_by_child_suffix(frame, &hand_prefix, finger, ".curl") {
			sum += value;
			count += 1;
		}
	}
...
fn finger_rotation(frame: &UNMotionFrame, bone: &str) -> Option<[f32; 4]> {
	...
	let prefix = side.signal_prefix();
	let hand_prefix = signal_prefix2("hand", prefix);
	let finger_prefix = signal_prefix(&hand_prefix, finger);
	...
	let curl = scalar_signal_by_child_suffix(frame, &finger_prefix, joint, ".curl")
		.or_else(|| scalar_signal_by_prefix_suffix(frame, &finger_prefix, ".curl"))
		.unwrap_or(0.0);
...
fn signal_prefix2(kind: &str, side: &str) -> String {
	let mut out = String::with_capacity(kind.len() + side.len() + 1);
	out.push_str(kind);
	out.push('.');
	out.push_str(side);
	out
...
fn hand_palm_signal_prefix(side: &str) -> String {
	let mut out = String::with_capacity(10 + side.len());
	out.push_str("hand.");
	out.push_str(side);
	out.push_str(".palm");
	out
...
fn finger_curl_to_quat(curl: f32, spread: f32, factor: f32, rest: &str, side: BodySide, finger_curl: f32) -> [f32; 4] {
	if rest == "ThumbProximal" {
		return thumb_proximal_to_quat(curl, spread, side, finger_curl);
	}
...
fn thumb_proximal_to_quat(curl: f32, spread: f32, side: BodySide, finger_curl: f32) -> [f32; 4] {
...
fn thumb_cm_yaw_from_canonical_spread(spread: f32, side: BodySide) -> f32 {
	let spread = spread.clamp(-1.0, 1.0);
...
fn thumb_signal_flexion_angle(curl: f32, rest: &str, side: BodySide) -> f32 {
...
fn default_blendshape_map_ref() -> &'static HashMap<String, BlendshapeRoute> {
	static DEFAULT_BLENDSHAPE_MAP: OnceLock<HashMap<String, BlendshapeRoute>> = OnceLock::new();
	DEFAULT_BLENDSHAPE_MAP.get_or_init(default_blendshape_map)
}
...
mod tests {
	...
	#[test]
	fn thumb_proximal_spread_applies_without_opposition_curl_gate() {
		let rotation = thumb_proximal_to_quat(0.05, 0.8, BodySide::Left, 0.05);
		assert!(
			rotation[1].abs() > 0.09,
			"open thumb spread should affect proximal rotation even without opposition: {rotation:?}"
		);
		assert!(
			rotation[1] < 0.0,
		...
		assert!(
			rotation[0].abs() < 0.05,
			"open spread alone should not trigger opposition pitch: {rotation:?}"
		);
	}
}
