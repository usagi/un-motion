...
#[derive(Clone, Debug, PartialEq)]
pub struct LatestMotionFrame {
	pub stream_id: StreamId,
	pub frame: Arc<UNMotionFrame>,
	pub state: Arc<StreamState>,
	pub snapshot: StreamSnapshot,
}

#[derive(Clone)]
pub struct LatestMotionFrameSlot {
	inner: Arc<ArcSwapOption<LatestMotionFrame>>,
}

impl LatestMotionFrameSlot {
	pub fn new() -> Self {
		Self {
	...
	pub fn store(&self, latest: LatestMotionFrame) {
		self.inner.store(Some(Arc::new(latest)));
	}

	pub fn load(&self) -> Option<Arc<LatestMotionFrame>> {
		self.inner.load_full()
	}
}

impl Default for LatestMotionFrameSlot {
	fn default() -> Self {
		Self::new()
	}
}

pub struct LatestMotionFrameStreamWorkerHandle {
	pub stream_id: StreamId,
	pub telemetry: Option<crate::SourceTelemetryHandle>,
	pub slot: LatestMotionFrameSlot,
	control_tx: Sender<MotionFrameStreamWorkerControl>,
	join: Option<JoinHandle<()>>,
}

impl LatestMotionFrameStreamWorkerHandle {
	pub fn stop(&self) {
...
impl Drop for LatestMotionFrameStreamWorkerHandle {
	fn drop(&mut self) {
		let _ = self.control_tx.send(MotionFrameStreamWorkerControl::Stop);
	}
}
...
pub fn spawn_latest_motion_frame_stream_worker<S>(
	config: MotionFrameStreamConfig,
	source: S,
	idle_sleep: Duration,
) -> anyhow::Result<LatestMotionFrameStreamWorkerHandle>
where
	S: MotionFrameSource,
{
	let telemetry = source.telemetry_handle();
	...
	let (control_tx, control_rx) = mpsc::channel();
	let thread_stream_id = stream_id.clone();
	let join = thread::spawn(move || run_latest_frame_stream_worker(&mut worker, control_rx, thread_slot, idle_sleep));
	Ok(LatestMotionFrameStreamWorkerHandle {
		stream_id: thread_stream_id,
		telemetry,
		slot,
		control_tx,
...
fn run_latest_frame_stream_worker<S>(
	worker: &mut MotionFrameStreamWorker<S>,
	control_rx: Receiver<MotionFrameStreamWorkerControl>,
	slot: LatestMotionFrameSlot,
	idle_sleep: Duration,
) where
	S: MotionFrameSource,
{
	loop {
		...
		match worker.poll_once() {
			Ok(mut poll) => {
				let idle = poll.is_idle();
				if let Some(frame) = poll.frames.pop() {
					slot.store(LatestMotionFrame {
						stream_id: poll.stream_id,
						frame: Arc::new(frame),
						state: Arc::new(poll.state),
...
pub fn apply_unmotion_frame_to_stream_state(state: &mut StreamState, frame: &UNMotionFrame, observed_at_ns: u64) {
	...
	let canonical_frame;
	let pose_frame = if frame_needs_signal_derived_motion(frame) {
		canonical_frame = {
	...
	let pose = vmc_frame_pose_for_unmotion_frame(pose_frame);
	let mut event_count = 0_u64;
	state.root = None;
	state.bones.clear();
	state.blendshapes.clear();
	state.part_diagnostics.clear();
	observe_unmotion_part_diagnostics(&mut state.part_diagnostics, pose_frame);
...
mod tests {
	...
	#[test]
	fn spawned_latest_worker_publishes_latest_slot_without_queueing() {
		let mut first = UNMotionFrame::new(7);
		first.face = Some(face_with_expression("jawOpen", 0.2));
		let mut second = UNMotionFrame::new(8);
		second.face = Some(face_with_expression("jawOpen", 0.8));
		let handle = spawn_latest_motion_frame_stream_worker(
			MotionFrameStreamConfig::new(StreamId::new("unmotion:stream-latest")),
			VecFrameSource::new(vec![first, second]),
			Duration::from_millis(1),
		)
		.expect("spawn latest worker");

		let deadline = std::time::Instant::now() + Duration::from_secs(1);
		let mut latest = None;
		while std::time::Instant::now() < deadline {
			latest = handle.slot.load();
			if latest.as_ref().is_some_and(|frame| frame.frame.header.sequence == 8) {
				break;
			}
		...
		let latest = latest.expect("latest frame");
		assert_eq!(latest.stream_id, StreamId::new("unmotion:stream-latest"));
		assert_eq!(latest.frame.header.sequence, 8);
		assert_eq!(latest.state.packet_count, 2);
		assert_eq!(latest.state.blendshapes["jawOpen"], 0.8);
	}
}
