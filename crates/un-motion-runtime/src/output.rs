...
impl VmcOutputWorker {
	...
	pub fn send_frame(&mut self, frame: VmcOutputFrame) -> anyhow::Result<VmcOutputEvent> {
		match frame {
			VmcOutputFrame::UnmotionFrame(mut frame) => {
				...
				self.modifier_pipeline.apply(&mut frame);
				let mut packets = vmc_packets_for_frame(&frame);
				retain_vmc_packets_for_modifier(&mut packets, &self.config.modifier);
				if !self.config.send_ok_packet {
					// `vmc_packets_for_frame` は OK packet を含めるので、設定で無効なら除去する。
					packets.retain(|p| !matches!(p, OscPacket::Message(message) if message.addr == "/VMC/Ext/OK"));
				}
