...
//!
//! Capturer の正式経路
//! `Input → Engine(MediaPipe + PostProcess) → UNMotionFrame → Modifier → Output`
...
//!   は `Smoothing → Mirror → BoneFilter` で、意味論的に「**ノイズを整える** → **視点を
//!   整える** → **不要を捨てる**」と読める並びを採る。
//!
...
/// デフォルトの段順: `NeutralCalibration → Smoothing → Mirror → BoneFilter`。
/// 意味論的に「ノイズを整える → 視点を整える → 不要を捨てる」と読める並び。
...
fn neutral_calibration_key_enabled(key: &str) -> bool {
	// Old profiles may still contain limb calibration entries. Do not apply them:
	// arm/hand rotations are pose outputs, not stable neutral offsets.
...
impl SmoothingStage {
	...
	fn step_expression(&mut self, name: &str, current: f32, timestamp_ns: u64, confidence: f32) -> f32 {
		let mode = self.mode;
		if let Some(state) = self.face_expression_state.get_mut(name) {
			Self::step_scalar(mode, state, current, timestamp_ns, confidence)
		} else {
			self.face_expression_state.insert(
				name.to_string(),
...
impl ModifierStage for SmoothingStage {
	...
	fn apply(&mut self, frame: &mut UNMotionFrame) {
		...
		for signal in frame.signals.iter_mut() {
			...
			let key = format!("signal:{}", signal.name);
			signal.value = MotionSignalValue::Scalar(self.step_expression(&key, value, timestamp_ns, signal.confidence));
		}
	}
}
...
mod tests {
	...
	fn pipeline_includes_smoothing_mirror_and_bone_filter() {
		// E-α-2 完了: Smoothing + Mirror + BoneFilter の 3 stage が `Smoothing →
	...
	fn neutral_calibration_removes_local_delta_from_rotation_tail() {
		let expected_swing = quat_axis_angle([0.0, 1.0, 0.0], 0.7);
		let neutral_delta = quat_axis_angle([1.0, 0.0, 0.0], 0.45);
		let captured = quat_normalize(quat_mul(expected_swing, neutral_delta));
		let mut rotations = HashMap::new();
		...
		assert_frame_quat_near(head_rotation(&frame), expected_swing, 1e-5);
	}

	#[test]
	fn neutral_calibration_ignores_limb_offsets_from_older_profiles() {
		...
		rotations.insert("LeftLowerArm".to_string(), quat_to_array(quat_axis_angle([0.0, 1.0, 0.0], 1.2)));
		let mut frame = UNMotionFrame::new(1);
		frame.left_hand = Some(HandMotion {
			tracking_state: TrackingState::Valid,
			confidence: 1.0,
