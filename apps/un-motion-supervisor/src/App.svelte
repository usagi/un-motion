...
<script lang="ts">
  ...
  function pickInitialLaunchTargetId(
    ...
    if (isValidLaunchTarget(active, items)) return active;
    return items[0]?.id ?? "";
  }
  ...
  async function playCalibrationSound(
    ...
    try {
      ...
      // Audio playback can be blocked by the webview; calibration itself must continue.
    }
  }
  ...
  const selectedCapturerProfileId = $derived(
    selectedCapturer?.profileId ?? activeProfileId ?? null,
  );
  ...
  async function refreshProfiles() {
    try {
      ...
      activeProfileId = await invoke<string>("active_profile_id");
      ...
      if (!stillExists) {
        selectedProfileId =
          profiles.find((profile) => profile.id === activeProfileId)?.id ??
          profiles[0]?.id ??
          null;
      }
  ...
  async function updateProfileField(field: string, value: unknown) {
    ...
    try {
      ...
      // 編集が稼働中の Capturer に PUT /api/profiles/document で反映されると、
      // Core 側は runtime を自動 restart する。UX を滑らかにするため telemetry を
      // 即時更新する (1.5s の polling を待たない)。
  ...
  async function restartCapturer(capturer: CapturerInstance) {
    ...
    const profile_id = capturer.profileId ?? activeProfileId ?? null;
    busy = true;
  ...
  function calibrationOverlay(
    ...
    const poseText =
      pose === "T" ? "Tポーズ" : pose === "I" ? "Iポーズ" : "Uポーズ";
    const body =
      pose === "T"
        ? [
            "実際の演者の姿勢と人型モデルの相対的な差を解消します。",
            "全身を映せる場合向けの調整用姿勢です。イラストを参考にTの字の姿勢で手の平は正面に向け、親指は自然に開いた状態にして下さい。",
          ]
        : pose === "I"
          ? [
              "実際の演者の姿勢と人型モデルの相対的な差を解消します。",
          ...
          : [
              "実際の演者の姿勢と人型モデルの相対的な差を解消します。",
              "胸上から頭部の周辺の比較的狭い映りで頭/顔/手/腕の姿勢推定に特化した調整用姿勢です。イラストを参考に手の平を正面に向け、手首を頭上に、首と手首と肘が同一平面上になる姿勢にして下さい。",
            ];
    return {
      kind: "calibration",
      ...
      highlights:
        pose === "T"
          ? ["Tの字の姿勢", "手の平は正面", "親指は自然に開く"]
          : pose === "I"
            ? ["Iの字の姿勢"]
            : ["手の平を正面", "手首を頭上", "首・手首・肘を同一平面"],
    };
  }
  ...
  function faceModelOverlay(targetLabel: string): ActionCountdownOverlay {
    return {
      ...
      phase: "prepare",
      body: [
        "演者固有の顔ランドマーク比率を測定し、Headの仰俯角推定を補正します。",
        "顔をカメラへ正面に向け、首を自然に立て、表情は軽くニュートラルにして下さい。作成後はHeadの見上げ/見下げ方向と飛び耐性が改善します。",
      ],
      highlights: ["顔を正面", "首を自然に立てる", "表情はニュートラル"],
    };
  }

  function unmfPoseOverlay(targetLabel: string): ActionCountdownOverlay {
    return {
      kind: "unmf-pose",
      title: "UNMFポーズ保存",
      subtitle: targetLabel,
      ...
      phase: "prepare",
      body: [
        "現在の姿勢を1フレームのUNMF/JSONLとして保存します。保存したい姿勢を作って静止して下さい。",
      ],
      highlights: ["保存したい姿勢で静止"],
    };
  }
  ...
  async function calibrateCapturerNeutral(
    ...
    message = `Calibration ${pose} #${capturer.id}: prepare pose...`;
    errorMessage = "";
    try {
      ...
      await refreshProfiles();
      await refreshRuntimeStatuses();
      message = `Calibration ${pose} saved for ${detail.name}`;
    } catch (error) {
  ...
  async function buildCapturerFacePoseModel(
    ...
    if (busy || calibratingCapturerId != null) return;
    calibratingCapturerId = capturer.id;
    message = `Face model #${capturer.id}: face front...`;
    errorMessage = "";
    try {
      await runActionCountdown(
        `Face model #${capturer.id}`,
        "sampling",
      ...
      message = `Face model saved for ${detail.name}${neutral == null ? "" : ` (${neutral.toFixed(3)})`}`;
    } catch (error) {
      errorMessage = `face model build failed: ${String(error)}`;
      message = "";
    } finally {
      actionCountdownOverlay = null;
      calibratingCapturerId = null;
  ...
  async function buildSelectedProfileFacePoseModel(): Promise<void> {
    ...
    busy = true;
    message = existing
      ? `Face model #${existing.id}: face front...`
      : `Launching ${profileName} for face model...`;
    errorMessage = "";
    try {
      if (!capturer) {
      ...
      if (shouldStopAfterBuild && capturer) {
        try {
          ...
          errorMessage = `face model saved, but temporary capturer stop failed: ${String(error)}`;
        }
      }
      busy = false;
    }
  ...
  async function saveCapturerUnmfPose(
    ...
    if (busy || calibratingCapturerId != null) return;
    calibratingCapturerId = capturer.id;
    message = `UNMF pose #${capturer.id}: prepare pose...`;
    errorMessage = "";
    try {
      await runActionCountdown(
        `UNMF pose #${capturer.id}`,
        "capturing",
        unmfPoseOverlay(`Capturer #${capturer.id}`),
      );
      ...
      errorMessage = `save UNMF pose failed: ${String(error)}`;
      message = "";
    } finally {
      actionCountdownOverlay = null;
      calibratingCapturerId = null;
  ...
  async function saveAllCapturersUnmfPose(): Promise<void> {
    if (busy || calibratingCapturerId != null) return;
    const targets = runningCapturers();
    if (targets.length === 0) return;
    busy = true;
    message = `UNMF pose ALL: prepare pose...`;
    errorMessage = "";
    try {
      await runActionCountdown(
        "UNMF pose ALL",
        "capturing",
        unmfPoseOverlay(`${targets.length} Capturers`),
      );
      await invoke<string | null>("save_all_capturers_unmf_pose");
      message = "";
    } catch (error) {
      errorMessage = `save ALL UNMF poses failed: ${String(error)}`;
      message = "";
    } finally {
      actionCountdownOverlay = null;
      busy = false;
  ...
  async function calibrateAllCapturersNeutral(
    ...
    if (targets.length === 0) return;
    busy = true;
    message = `Calibration ${pose} ALL: prepare pose...`;
    errorMessage = "";
    try {
      await runActionCountdown(
        `Calibration ${pose} ALL`,
        "sampling",
      ...
      await refreshProfiles();
      await refreshRuntimeStatuses();
      message = `Calibration ${pose} saved for ${targets.length} capturers`;
    } catch (error) {
      errorMessage = `calibrate ALL ${pose} failed: ${String(error)}`;
      message = "";
    } finally {
      actionCountdownOverlay = null;
      busy = false;
  ...
  function profileNeedsFacePoseModel(
    ...
    if ((runtime.modifierHeadEnabled ?? true) === false) return false;
    return !(
  ...
  // ===================================================================
  // Webcam device / format enumeration
  //
  ...
  async function launchCapturer() {
    ...
    try {
      ...
      const profile_id = launchProfileId || activeProfileId || null;
      const instance = await invoke<CapturerInstance>("launch_capturer", {
        profileId: profile_id,
        allowNonLoopback: false,
      });
  ...
  async function applyActiveProfile(profileId: string) {
    if (busy) return;
    if (!profileId || profileId === activeProfileId) return;
    busy = true;
    message = `Switching to ${profileId}...`;
    errorMessage = "";
    try {
      activeProfileId = await invoke<string>("set_active_profile", {
        profileId,
      });
      launchTargetId = activeProfileId;
      message = `Active profile set to ${activeProfileId}`;
      await refreshRuntimeStatuses();
    } catch (error) {
      errorMessage = `set_active_profile failed: ${String(error)}`;
      message = "";
    } finally {
      busy = false;
    }
        ...
        <div class="toolbar">
          ...
          <div class="toolbar-action-group">
            <button
              ...
              title="Save one UNMF pose from all running capturers"
              disabled={busy ||
                calibratingCapturerId != null ||
                runningCapturers().length === 0}
              onclick={() => void saveAllCapturersUnmfPose()}
            >
              ①:ALL
            </button>
            <button
            ...
            {#each ["U", "T", "I"] as pose (pose)}
              <button
                type="button"
                class="compact-action-button"
                title={`${pose}-pose calibration for all running capturers`}
                disabled={busy ||
                  calibratingCapturerId != null ||
                  runningCapturers().length === 0}
                onclick={() =>
                  void calibrateAllCapturersNeutral(
                    pose as CalibrationPoseKind,
                  )}
              >
                {pose}:ALL
              </button>
            ...
            <table class="process-table capturer-process-table">
              ...
              <tbody>
                {#each capturerTableCapturers as capturer (capturer.id)}
                  ...
                  {@const capProfileId =
                    capturer.profileId ?? activeProfileId ?? null}
                  {@const capProf =
                    capProfileId != null
            ...
            {#if selectedCapturer}
              ...
              <div class="inspector-command-bar">
                <button
                  ...
                  onclick={() => void saveCapturerUnmfPose(selectedCapturer)}
                  title="Save current UNMF pose"
                  >① Snapshot</button
                >
                <button
                  type="button"
                ...
                {#each ["U", "T", "I"] as pose (pose)}
                  <button
                    ...
                    title={`${pose}-pose calibration`}
                    >{pose} Cal</button
                  >
                {/each}
                <button
                  type="button"
                  class={profileNeedsFacePoseModel(runtime, capturerEngine)
                    ? "needs-face-model"
                    : ""}
                  disabled={busy ||
                    calibratingCapturerId != null ||
                    selectedCapturer.state !== "running"}
                  onclick={() => void buildCapturerFacePoseModel(selectedCapturer)}
                  title="Build face pose model"
                  >F Model</button
                >
              ...
              >
                <svg viewBox="0 0 260 320" role="img">
                  {#each [["Head", 130, 42, 130, 88], ["Torso", 130, 92, 130, 170], ["R-Arm", 96, 96, 54, 170], ["L-Arm", 164, 96, 206, 170], ["R-Hand", 54, 170, 38, 106], ["L-Hand", 206, 170, 222, 106], ["R-Leg", 120, 174, 92, 260], ["L-Leg", 140, 174, 168, 260], ["R-Foot", 92, 260, 78, 294], ["L-Foot", 168, 260, 190, 292]] as item (item[0])}
                    ...
                    <line
                      class={`diag-bone ${diagnosticClass(diag)}`}
                      x1={item[1]}
                      y1={item[2]}
                      x2={item[3]}
                      y2={item[4]}
                  ...
                  {/each}
                  <polyline
                    class="diag-body diag-high"
                    points="92,92 168,92 146,174 114,174 92,92"
                  />
                  {#each [["Head", 130, 24], ["Face", 130, 62], ["R-Hand", 18, 92], ["L-Hand", 196, 92], ["R-Arm", 38, 162], ["L-Arm", 184, 162], ["Torso", 104, 124], ["R-Leg", 66, 252], ["L-Leg", 154, 252], ["R-Foot", 46, 304], ["L-Foot", 170, 304]] as label (label[0])}
                    {@const diag = diagnosticForPart(
                      diagnosticSnapshot,
                      runtime,
                      label[0] as string,
                    ...
                    <text
                      class={`diag-label ${diagnosticClass(diag)} ${label[0] === "Head" || label[0] === "Face" ? "diag-label-center" : ""}`}
                      x={label[1]}
                      y={label[2]}
                    >
                      <tspan x={label[1]} dy="0">{label[0]}</tspan>
                      <tspan x={label[1]} dy="16">
                        {statusLabel(diag.status)}
                        {confidencePercent(diag)}%
                      </tspan>
                      {#if rate}
                        <tspan x={label[1]} dy="16">{rate}</tspan>
                      {/if}
                    </text>
                  {/each}
                </svg>
              </div>
              {#if profileNeedsFacePoseModel(runtime, capturerEngine)}
                <div class="face-model-warning" role="status">
                  <strong
                    >フェイス姿勢補正モデルの作成 [F]
                    が未完了のため推定精度が低くなる可能性があります。<br
                    />プロファイル起動中に Capturers タブから操作ボタン [F]
                    で作成できます。</strong
                  >
                </div>
              {/if}
      ...
      >
        <div class="toolbar">
          ...
          <span class="toolbar-message">
            {$_("profiles.actions.active_label")}
            <strong
              >{activeProfileId || $_("profiles.actions.active_unset")}</strong
            >
          </span>
        </div>
              ...
              >
                    ...
                    <div class="section-grid">
                      ...
                      {#if mediaPipeSourceType === "file-video" || mediaPipeSourceType === "webcam-directshow" || mediaPipeSourceType === "webcam-mediafoundation"}
                        ...
                        {#if mediaPipeSourceType === "file-video"}
                          <label
                            class="toggle-field"
                            data-hint="Loop video: 動画ファイルを最後まで再生したあと先頭から繰り返します。長時間の検証や調整に使います。"
                          >
                            <input
                              type="checkbox"
                            ...
                            />
                            <span>{$_("profiles.editor.loop_video")}</span>
                          </label>
                        {/if}
                      {/if}
                    </div>
                ...
                >
                  ...
                  <div class="modifier-groups">
                    ...
                    >
                      <h4>MediaPipe Post-process</h4>
                      {#if !isMediaPipeEngine}
                        <small class="muted-small"
                          >MediaPipe Native engine の場合のみ設定できます。</small
                        >
                      {/if}
                      <fieldset
                      ...
                      <div class="modifier-filter-grid">
                        {#each [{ field: "ease_recovery", label: "Ease recovery", value: profileDetail.runtime.modifierEaseRecovery, hint: "見失い復帰時の急な跳ねを緩和します。" }, { field: "limit_rotation_jumps", label: "Limit jumps", value: profileDetail.runtime.modifierLimitRotationJumps, hint: "不自然に大きい回転ジャンプを抑制します。" }, { field: "head_source_switch_blend", label: "Head source blend", value: profileDetail.runtime.modifierHeadSourceSwitchBlend, hint: "顔と姿勢の頭部推定ソースが切り替わるときの段差を緩和します。" }] as toggle (toggle.field)}
                          <label class="toggle-field" data-hint={toggle.hint}>
                            <input
                              type="checkbox"
                      ...
                      <div class="developer-face-model">
                        <div class="developer-face-model-row">
                          <label
                            class="toggle-field"
                            data-hint="フェイス姿勢補正モデル: 演者ごとの正面顔モデルで頭部姿勢を補正します。未作成の状態でチェックONにすると、このプロファイルで作成します。"
                          >
                            <input
                              type="checkbox"
                              checked={profileDetail.runtime
                        ...
                        {#if isMediaPipeEngine && profileDetail.runtime.modifierFacePoseModelNeutralNoseDropEyeMouth == null}
                          <div
                            class="face-model-warning profile-face-model-warning"
                            role="status"
                          >
                            <strong
                              >フェイス姿勢補正モデルの作成が未完了のため推定精度が低くなる可能性があります。このチェックをONにすると、このプロファイルで作成できます。</strong
                            >
                          </div>
                        {/if}
                      </div>
                ...
                >
                  ...
                  <div class="subgroup">
                    <label
                      ...
                      data-hint="UNMF/Z Publisher: UNMotionFrameをZenohで送出します。UNAvatarなどUNMF/Z対応アプリへ送る標準出力です。通常はONです。"
                    >
                      <input
                        type="checkbox"
                  ...
                  <div class="subgroup">
                    <label
                      class="output-channel-heading"
                      data-hint="VMC/UDP Sender: VMC受信対応アプリへ互換出力する場合に使います。UNMF/Zだけを使う場合はOFFで十分です。"
                    >
                      <input
                        type="checkbox"
