...
/// * `<user_dir>/conf.toml`     — `[desktop]` セクション (active_profile_id 等)
...
///
/// Capturer は `--auto-start` を既定で受け取って起動と同時に `selected_profile_id` で
/// runtime を立ち上げる。Supervisor が `profile_id` を指定した場合は healthz 後に
/// `PUT /api/profiles/active` を呼んで対象 profile に切替える (Capturer 内部で自動 restart)。
...
fn launch_capturer(
	...
	// profile_id が明示されていれば、Capturer が auto-start した既定 profile から切替える。
	// 失敗しても Capturer は生かしておく (GUI 側のリトライ / Restart で復旧可能)。
	if let Some(ref pid) = resolved_profile_id {
		if let Err(error) = apply_active_profile_to_capturer(&state.http_client, bind_addr, pid) {
			tracing::warn!(profile_id = %pid, %error, "launch_capturer: failed to apply requested profile (capturer keeps running on default)");
		}
	}
...
/// Capturer の `PUT /api/profiles/active` を 1 回叩いて active profile を切替える小さなヘルパ。
/// auto-start で立ち上がった既定 profile から、Supervisor で選択していた profile へ
/// 移行するために `launch_capturer` から呼ぶ。
fn apply_active_profile_to_capturer(
	http_client: &reqwest::blocking::Client,
	bind_addr: SocketAddr,
	profile_id: &str,
) -> Result<(), String> {
	let url = format!("http://{}/api/profiles/active", bind_addr);
	let body = serde_json::json!({ "profileId": profile_id });
	...
	if !response.status().is_success() {
		return Err(format!("PUT {url} HTTP {}", response.status()));
	}
	Ok(())
}
...
fn calibrate_capturer_neutral(
	...
	let profile_id = capturer
		...
		.profile_id
		.clone()
		.or_else(|| first_running_capturer_active_profile_id(&state))
	...
	let saved = store
		.save(envelope.selection)
		.map_err(|error| format!("save profile document after calibration: {error}"))?;
	saved
		.profiles
		.iter()
...
fn clear_capturer_neutral_calibration(id: u32, state: State<'_, Mutex<SupervisorState>>) -> Result<ProfileDetail, String> {
	...
	let saved = store
		.save(envelope.selection)
		.map_err(|error| format!("save profile document after calibration clear: {error}"))?;
	saved
		.profiles
		.iter()
		.find(|profile| profile.id == profile_id)
...
fn build_capturer_face_pose_model(
	...
	let saved = store
		.save(envelope.selection)
		.map_err(|error| format!("save profile document after face pose model build: {error}"))?;
	saved
		.profiles
		.iter()
		.find(|profile| profile.id == profile_id)
...
#[tauri::command]
fn save_capturer_unmf_pose(id: u32, state: State<'_, Mutex<SupervisorState>>) -> Result<Option<String>, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	let capturer = state.capturers.get(&id).ok_or_else(|| format!("capturer not found: {id}"))?;
	if !matches!(capturer.info.state, CapturerState::Running | CapturerState::Starting) {
		return Err(format!("capturer is not running: {id}"));
	}
	let frame = capture_unmf_pose_from_capturer(&state.http_client, capturer.bind_addr)?;
	let ts = std::time::SystemTime::now()
		.duration_since(std::time::UNIX_EPOCH)
		.map(|d| d.as_secs())
		.unwrap_or(0);
	save_unmf_jsonl(vec![frame], format!("un-motion-pose-{ts}.unmf.jsonl"))
}

#[tauri::command]
fn save_all_capturers_unmf_pose(state: State<'_, Mutex<SupervisorState>>) -> Result<Option<String>, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	let targets = state
		.capturers
		.values()
	...
	for bind_addr in targets {
		frames.push(capture_unmf_pose_from_capturer(&state.http_client, bind_addr)?);
	}
	let ts = std::time::SystemTime::now()
		.duration_since(std::time::UNIX_EPOCH)
		.map(|d| d.as_secs())
		.unwrap_or(0);
	save_unmf_jsonl(frames, format!("un-motion-poses-all-{ts}.unmf.jsonl"))
}
...
fn save_unmf_jsonl(frames: Vec<serde_json::Value>, default_file_name: String) -> Result<Option<String>, String> {
...
/// 現在 active な profile id を返す。稼働 Capturer があればそこから、なければ workspace
/// `conf.toml` の `selectedProfileId` を返す。
#[tauri::command]
fn active_profile_id(state: State<'_, Mutex<SupervisorState>>) -> Result<String, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	if let Some(active) = first_running_capturer_active_profile_id(&state) {
		return Ok(active);
	}
	Ok(user_profile_store().load().selected_profile_id)
}

/// Profile を全 Capturer に切り替え、workspace `conf.toml` の `selectedProfileId` も更新する。
/// 個別 Capturer の HTTP 失敗はログに残すが、最低 1 回はローカルへ書き戻して GUI の状態を
/// 一貫させる。
#[tauri::command]
fn set_active_profile(profile_id: String, state: State<'_, Mutex<SupervisorState>>) -> Result<String, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	let body = serde_json::json!({ "profileId": &profile_id });
	for capturer in state.capturers.values() {
		...
		let url = format!("http://{}/api/profiles/active", capturer.bind_addr);
		if let Err(error) = state.http_client.put(&url).json(&body).send() {
			tracing::warn!(id = capturer.info.id, %error, "set_active_profile via capturer failed");
		}
	}
	let store = user_profile_store();
	let mut doc = store.load();
	doc.selected_profile_id = profile_id.clone();
...
/// を OFF で開始する。
#[tauri::command]
fn create_profile(name: String, state: State<'_, Mutex<SupervisorState>>) -> Result<ProfileDetail, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	...
	push_document_to_running_capturers(&state, &saved);
	let detail = saved
		.profiles
		.iter()
...
/// Profile を削除する。最後の 1 件も削除できる。空になったら active は空文字にする。
#[tauri::command]
...
#[tauri::command]
fn duplicate_profile(profile_id: String, state: State<'_, Mutex<SupervisorState>>) -> Result<ProfileDetail, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	let store = user_profile_store();
	let mut doc = store.load();
	let original = doc
...
#[tauri::command]
fn reorder_profiles(profile_ids: Vec<String>, state: State<'_, Mutex<SupervisorState>>) -> Result<Vec<ProfileSummary>, String> {
	let state = state.lock().map_err(|_| "supervisor state poisoned".to_string())?;
	let store = user_profile_store();
	let mut doc = store.load();
...
impl From<&CoreProfileDocumentProfile> for ProfileDetail {
	fn from(profile: &CoreProfileDocumentProfile) -> Self {
		let runtime = profile.runtime_selection.as_ref().map(ProfileRuntimeView::from).unwrap_or_default();
...
/// 保存済み document を稼働中の Capturer に PUT /api/profiles/document で反映する。
/// 失敗は警告のみで握りつぶす (再起動すれば conf.toml から復元できるため致命的でない)。
fn push_document_to_running_capturers(state: &SupervisorState, doc: &CoreProfileDocument) {
	let body = serde_json::json!({ "selection": doc });
	for capturer in state.capturers.values() {
		if !matches!(capturer.info.state, CapturerState::Running) {
			continue;
		}
		let url = format!("http://{}/api/profiles/document", capturer.bind_addr);
		if let Err(error) = state.http_client.put(&url).json(&body).send() {
			tracing::warn!(id = capturer.info.id, %error, "push profile document via capturer failed");
		}
	}
}

/// 稼働中の Capturer から HTTP 経由で active_profile_id を取得する。
fn first_running_capturer_active_profile_id(state: &SupervisorState) -> Option<String> {
	let capturer = state.capturers.values().find(|c| matches!(c.info.state, CapturerState::Running))?;
	let url = format!("http://{}/api/profiles/active", capturer.bind_addr);
	let response = state.http_client.get(&url).send().ok()?;
	if !response.status().is_success() {
		return None;
	}
	#[derive(serde::Deserialize)]
