...
MediaPipe Native の場合、Engine は推論と PostProcess の両方を含みます。

```text
Image input -> MediaPipe Native -> MediaPipe PostProcess -> UNMotionFrame
...
- MediaPipe PostProcess は MediaPipe Engine 境界の一部です。Engine 内部構造を Output へ漏らしません。
...
- Output が MediaPipe raw output や PostProcess 専用データを直接読む。
