...
- `vrcOscParameterPrefix`: VRCFaceTracking parameter prefix です。空なら標準の `v2/...` を使います。
...
OSC argument は MVP では Float を基本とします。
...
実装は VMC output の blendshape map と同じ発想で、入力 signal 名から VRCFT parameter 名への route を持ちます。
...
face.mouthSmileLeft -> v2/MouthSmileLeft
face.mouthSmileRight -> v2/MouthSmileRight
