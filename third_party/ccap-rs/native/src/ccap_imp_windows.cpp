...
VideoMediaTypeInfo videoMediaTypeInfo(const AM_MEDIA_TYPE* mediaType) {
    VideoMediaTypeInfo out;
    if (!mediaType || !mediaType->pbFormat) {
        return out;
    }
    if (mediaType->formattype == FORMAT_VideoInfo && mediaType->cbFormat >= sizeof(VIDEOINFOHEADER)) {
        auto* videoHeader = reinterpret_cast<VIDEOINFOHEADER*>(mediaType->pbFormat);
        out.width = videoHeader->bmiHeader.biWidth;
        out.rawHeight = videoHeader->bmiHeader.biHeight;
        out.height = std::abs(videoHeader->bmiHeader.biHeight);
        out.bitCount = videoHeader->bmiHeader.biBitCount;
        out.fps = videoHeader->AvgTimePerFrame != 0 ? 10000000.0 / videoHeader->AvgTimePerFrame : 0.0;
    } else if (mediaType->formattype == FORMAT_VideoInfo2 && mediaType->cbFormat >= sizeof(VIDEOINFOHEADER2)) {
        auto* videoHeader = reinterpret_cast<VIDEOINFOHEADER2*>(mediaType->pbFormat);
        out.width = videoHeader->bmiHeader.biWidth;
        out.rawHeight = videoHeader->bmiHeader.biHeight;
        out.height = std::abs(videoHeader->bmiHeader.biHeight);
        out.bitCount = videoHeader->bmiHeader.biBitCount;
...
double mediaTypeFps(const AM_MEDIA_TYPE* mediaType) {
    return videoMediaTypeInfo(mediaType).fps;
}

double fpsDistance(double desiredFps, const AM_MEDIA_TYPE* mediaType) {
    if (desiredFps <= 0.0) {
        return 0.0;
    }
    double fps = mediaTypeFps(mediaType);
    if (fps <= 0.0) {
        return std::numeric_limits<double>::max() / 4.0;
    }
    return std::abs(fps - desiredFps);
}

bool streamCapsSupportsSize(const VIDEO_STREAM_CONFIG_CAPS& caps, int width, int height) {
    if (width <= 0 || height <= 0) {
        return false;
    }
    if (width < caps.MinOutputSize.cx || width > caps.MaxOutputSize.cx || height < caps.MinOutputSize.cy || height > caps.MaxOutputSize.cy) {
        return false;
    }
    LONG granularityX = caps.OutputGranularityX > 0 ? caps.OutputGranularityX : 1;
    LONG granularityY = caps.OutputGranularityY > 0 ? caps.OutputGranularityY : 1;
    return ((width - caps.MinOutputSize.cx) % granularityX) == 0 && ((height - caps.MinOutputSize.cy) % granularityY) == 0;
}

bool streamCapsSupportsFps(const VIDEO_STREAM_CONFIG_CAPS& caps, double fps) {
    if (fps <= 0.0) {
        return true;
    }
    auto frameInterval = static_cast<LONGLONG>(std::llround(10000000.0 / fps));
    return frameInterval >= caps.MinFrameInterval && frameInterval <= caps.MaxFrameInterval;
}

void setVideoMediaTypeSizeAndFps(AM_MEDIA_TYPE* mediaType, int width, int height, double fps) {
    if (!mediaType || !mediaType->pbFormat) {
        return;
    }
    auto frameInterval = fps > 0.0 ? static_cast<REFERENCE_TIME>(std::llround(10000000.0 / fps)) : 0;
    if (mediaType->formattype == FORMAT_VideoInfo && mediaType->cbFormat >= sizeof(VIDEOINFOHEADER)) {
        auto* videoHeader = reinterpret_cast<VIDEOINFOHEADER*>(mediaType->pbFormat);
        videoHeader->bmiHeader.biWidth = width;
        videoHeader->bmiHeader.biHeight = videoHeader->bmiHeader.biHeight < 0 ? -height : height;
        if (frameInterval > 0) videoHeader->AvgTimePerFrame = frameInterval;
    } else if (mediaType->formattype == FORMAT_VideoInfo2 && mediaType->cbFormat >= sizeof(VIDEOINFOHEADER2)) {
        auto* videoHeader = reinterpret_cast<VIDEOINFOHEADER2*>(mediaType->pbFormat);
        videoHeader->bmiHeader.biWidth = width;
        videoHeader->bmiHeader.biHeight = videoHeader->bmiHeader.biHeight < 0 ? -height : height;
...
void printMediaType(AM_MEDIA_TYPE* pmt, const char* prefix) {
    ...
    const char* rangeStr = "";
    auto videoInfo = videoMediaTypeInfo(pmt);
    ...
    printf("%s%ld x %ld  bitcount=%ld  format=%s%s, fps=%g\n", prefix, videoInfo.width, videoInfo.height, videoInfo.bitCount, info.name,
           rangeStr, videoInfo.fps);
    fflush(stdout);
}
...
std::unique_ptr<ProviderDirectShow::MediaInfo> ProviderDirectShow::enumerateMediaInfo(
    ...
    auto& videoCaps = mediaInfo->videoCaps;
    ...
    if (SUCCEEDED(hr) && streamConfig) {
        ...
        for (int i = 0; i < capabilityCount; ++i) {
            ...
            if (SUCCEEDED(streamConfig->GetStreamCaps(i, &mediaType, capabilityData.data()))) {
                if ((mediaType->formattype == FORMAT_VideoInfo || mediaType->formattype == FORMAT_VideoInfo2) && mediaType->pbFormat) {
                    videoMediaTypes.emplace_back(mediaType);
                    VIDEO_STREAM_CONFIG_CAPS caps{};
                    if (capabilitySize >= static_cast<int>(sizeof(VIDEO_STREAM_CONFIG_CAPS))) {
                        std::memcpy(&caps, capabilityData.data(), sizeof(VIDEO_STREAM_CONFIG_CAPS));
                    }
                    videoCaps.emplace_back(caps);
                    if (callback) {
                        auto videoInfo = videoMediaTypeInfo(mediaType);
                        auto info = findPixelFormatInfo(mediaType->subtype);
                        if (callback(mediaType, info.name, info.pixelFormat, { (uint32_t)videoInfo.width, (uint32_t)videoInfo.height })) {
...
bool ProviderDirectShow::createStream() {
    ...
    if (auto mediaInfo = enumerateMediaInfo(nullptr)) {
        ...
        const double desiredFps = m_frameProp.fps;
        CCAP_LOG_V("ccap: Desired capture format: %d x %d, fps=%g\n", desiredWidth, desiredHeight, desiredFps);
        ...
        auto capsFor = [&](AM_MEDIA_TYPE* mediaType) -> const VIDEO_STREAM_CONFIG_CAPS* {
            for (size_t i = 0; i < videoTypes.size() && i < videoCaps.size(); ++i) {
                if (videoTypes[i] == mediaType) {
                    return &videoCaps[i];
                }
            }
            return nullptr;
        ...
        auto supportsDesiredSize = [&](AM_MEDIA_TYPE* mediaType) {
            const auto* caps = capsFor(mediaType);
            return caps && streamCapsSupportsSize(*caps, desiredWidth, desiredHeight);
        };

        for (auto* mediaType : videoTypes) {
            auto videoInfo = videoMediaTypeInfo(mediaType);
            if ((desiredWidth <= videoInfo.width && desiredHeight <= videoInfo.height) || supportsDesiredSize(mediaType)) {
        ...
        for (auto* mediaType : matchedTypes) {
            if ((mediaType->formattype == FORMAT_VideoInfo || mediaType->formattype == FORMAT_VideoInfo2) && mediaType->pbFormat) {
                auto videoInfo = videoMediaTypeInfo(mediaType);
                double width = supportsDesiredSize(mediaType) ? desiredWidth : static_cast<double>(videoInfo.width);
                double height = supportsDesiredSize(mediaType) ? desiredHeight : static_cast<double>(videoInfo.height);
        ...
        if (!bestMatchedTypes.empty()) { // Resolution is closest, now try to select a suitable format.
            ...
            std::vector<AM_MEDIA_TYPE*> formatMatchedTypes;
            for (auto* candidate : bestMatchedTypes) {
                auto pixFormatInfo = findPixelFormatInfo(candidate->subtype);
                if (pixFormatInfo.pixelFormat == preferredPixelFormat ||
                    (!(preferredPixelFormat & kPixelFormatYUVColorBit) && pixFormatInfo.subtype == MEDIASUBTYPE_MJPG)) {
                    formatMatchedTypes.emplace_back(candidate);
                }
            }

            auto& fpsCandidates = formatMatchedTypes.empty() ? bestMatchedTypes : formatMatchedTypes;
            auto rightFormat = std::min_element(fpsCandidates.begin(), fpsCandidates.end(), [&](AM_MEDIA_TYPE* lhs, AM_MEDIA_TYPE* rhs) {
                return fpsDistance(desiredFps, lhs) < fpsDistance(desiredFps, rhs);
            });

            if (rightFormat != fpsCandidates.end()) {
                mediaType = *rightFormat;
            }

            if (mediaType == nullptr) {
                // When format is YUV, only one suitable format can be found
                rightFormat = std::find_if(bestMatchedTypes.begin(), bestMatchedTypes.end(), [&](AM_MEDIA_TYPE* mediaType) {
            ...
            auto videoInfo = videoMediaTypeInfo(mediaType);
            if (supportsDesiredSize(mediaType)) {
                setVideoMediaTypeSizeAndFps(mediaType, desiredWidth, desiredHeight, desiredFps);
                videoInfo = videoMediaTypeInfo(mediaType);
            }
            m_frameProp.width = videoInfo.width;
            m_frameProp.height = videoInfo.height;
            m_frameProp.fps = videoInfo.fps;
            auto pixFormatInfo = findPixelFormatInfo(mediaType->subtype);
    ...
    hr = m_captureBuilder->RenderStream(&PIN_CATEGORY_CAPTURE, &MEDIATYPE_Video, m_deviceFilter, m_sampleGrabberFilter, m_dstNullFilter);
    if (FAILED(hr)) {
...
HRESULT STDMETHODCALLTYPE ProviderDirectShow::SampleCB(double sampleTime, IMediaSample* mediaSample) {
    ...
    if (!m_firstFrameArrived) {
        ...
        if (SUCCEEDED(hr)) {
            auto videoInfo = videoMediaTypeInfo(&mt);
            m_frameProp.width = videoInfo.width;
...
bool ProviderDirectShow::start() {
    ...
    if (m_propertyChanged && !m_isRunning && !m_deviceName.empty()) {
